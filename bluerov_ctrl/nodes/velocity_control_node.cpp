...
namespace bluerov_ctrl {
    AUVVelocityControlNode::AUVVelocityControlNode() : Node("auv_velocity_control_node") {
    ...
    void AUVVelocityControlNode::initController() {
        ros_param_utils::getParam(this, controller_type_, "controller_type", int(ControllerType::vel_pid_comp));
        switch (controller_type_) {
            ...
            case (ControllerType::vel_model_based_comp):
                ...
                break;
            default:
                RCLCPP_ERROR(this->get_logger(), "%s", ("Requested controller of type " +
                                                        std::to_string(controller_type_) + "not found!").c_str());
                break;
        }
    ...
    hippo_msgs::msg::ActuatorSetpoint AUVVelocityControlNode::zeroMsg(rclcpp::Time _stamp) {
    ...
    void AUVVelocityControlNode::onSetpointTarget(const hippo_msgs::msg::VelocityControlTarget::SharedPtr _msg) {
        if (_msg->header.frame_id != hippo_common::tf2_utils::frame_id::BaseLink(this)) {
            RCLCPP_WARN_THROTTLE(
                    get_logger(), *get_clock(), 1000, "%s",
                    ("Control Target frame is " + _msg->header.frame_id + "but only " +
                     hippo_common::tf2_utils::frame_id::BaseLink(this) + "is handled. Ignoring...").c_str());
            return;
        }
    ...
    void AUVVelocityControlNode::onOdometry(const nav_msgs::msg::Odometry::SharedPtr _msg) {
        if (!got_first_setpoint_) {
            return;
        }
        ...
        if (!estimation_feasible_){
            ...
            controller_interface_->getControllerOutput(dt, _msg, thrust_msg, torque_msg);
        }
        thrust_pub_->publish(thrust_msg);
        torque_pub_->publish(torque_msg);
        last_time_ = this->now();
