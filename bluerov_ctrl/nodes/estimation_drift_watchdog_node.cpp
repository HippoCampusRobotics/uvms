...
namespace bluerov_ctrl {
    ...
    class AUVEstimationWatchdogNode : public rclcpp::Node {
    public:
        AUVEstimationWatchdogNode() : Node("estimation_drift_watchdog_node"){
    ...
    private:
        void declareParams(){
            ...
            descr_text =
                    "Maximum absolute position error before shutdown.";
            ...
            descr_text =
                    "Maximum absolute attitude error before shutdown.";
            descr = hippo_common::param_utils::Description(descr_text, true);
        ...
        void initPublishers(){
            ...
            shutdown_pub_ = create_publisher<std_msgs::msg::Bool>(
        ...
        void initSubscriptions(){
            ...
            odometry_sub_ = create_subscription<nav_msgs::msg::Odometry>(
                    topic, qos, std::bind(&AUVEstimationWatchdogNode::onOdometry, this, _1));
        ...
        void onSetpointTarget(const hippo_msgs::msg::ControlTarget::SharedPtr _msg){
            if (_msg->header.frame_id != hippo_common::tf2_utils::frame_id::kInertialName) {
                RCLCPP_WARN_THROTTLE(
                        ...
                        ("Target frame is " + _msg->header.frame_id + "but only " + hippo_common::tf2_utils::frame_id::kInertialName + "is handled. Ignoring...").c_str());
                return;
            }
        ...
        Eigen::Vector3d quaternionError(const Eigen::Quaterniond &des, const Eigen::Quaterniond &state) {
            Eigen::Matrix3d v_des_tilde;
            skew(des.vec(), v_des_tilde);
            double scalar_error = des.w() * state.w() + des.vec().transpose() * state.vec();
            Eigen::Vector3d out = state.w() * des.vec() - des.w() * state.vec() - v_des_tilde * state.vec();
        ...
        void onOdometry(const nav_msgs::msg::Odometry::SharedPtr _msg){
            ...
            if (!got_first_odometry_){
                got_first_odometry_ = true;
                return;
            }
            Eigen::Vector3d position;
            Eigen::Quaterniond attitude;
            hippo_common::convert::RosToEigen(_msg->pose.pose.position, position);
            hippo_common::convert::RosToEigen(_msg->pose.pose.orientation, attitude);
            const double position_error = (mask_.segment<3>(0).array() *
                                           (position_des_ - position).array()).matrix().norm();
            const double attitude_error = (mask_.segment<3>(3).array() * (attitude_des_.toRotationMatrix().inverse() *
                    quaternionError(attitude_des_, attitude)).array()).matrix().norm();
            if (feasible_ && (position_error > max_position_error_ || attitude_error > max_attitude_error_)){
                feasible_ = false;
            }
        ...
        bool got_first_odometry_{false};

        //////////////////////////////////////////////////////////////////////////////
        // publishers
        //////////////////////////////////////////////////////////////////////////////
        rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr shutdown_pub_;
        rclcpp::Publisher<std_msgs::msg::Float64>::SharedPtr position_error_debug_pub_;
        rclcpp::Publisher<std_msgs::msg::Float64>::SharedPtr attitude_error_debug_pub_;
...
int main(int argc, char **argv){
    ...
    rclcpp::spin(std::make_shared<bluerov_ctrl::AUVEstimationWatchdogNode>());
    rclcpp::shutdown();
}
