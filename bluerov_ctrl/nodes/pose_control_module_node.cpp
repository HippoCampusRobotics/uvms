...
class AUVPoseControlModuleNode : public rclcpp::Node {
 ...
 private:
  ...
  void onSetpointTarget(const hippo_msgs::msg::ControlTarget::SharedPtr _msg) {
    if (_msg->header.frame_id !=
      ...
      RCLCPP_WARN_THROTTLE(
          ...
          ("Control Targe frame is " + _msg->header.frame_id + "but only " +
           hippo_common::tf2_utils::frame_id::kInertialName +
           "is handled. Ignoring...")
              .c_str());
      return;
  ...
  void onOdometry(const nav_msgs::msg::Odometry::SharedPtr _msg) {
    ...
    pos_control_module_interface->update(_msg->pose.pose.position,
                                         _msg->pose.pose.orientation,
                                         velocity_setpoint_msg.velocity.linear);
    att_control_module_interface->update(
        _msg->pose.pose.orientation, velocity_setpoint_msg.velocity.angular);
