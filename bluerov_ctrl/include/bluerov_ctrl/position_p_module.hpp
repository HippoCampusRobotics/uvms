...
namespace bluerov_ctrl {
    ...
    public:
        ...
        void setPgainX(const double &gain) {
            p_gains_.x() = gain;
        }

        void setPgainY(const double &gain) {
            p_gains_.y() = gain;
        }

        void setPgainZ(const double &gain) {
            p_gains_.z() = gain;
        }
        ...
        void update(const Eigen::Vector3d &pos, const Eigen::Quaterniond &att, Eigen::Vector3d &out_vel);

    private:
        Eigen::Array3d p_gains_{1.0, 1.0, 1.0};
        Eigen::Vector3d pos_target{0.0, 0.0, 0.0};  //!< position target in world frame
        Eigen::Vector3d vel_target{0.0, 0.0, 0.0};  //!< velocity target in world frame
        Eigen::DiagonalMatrix<double, 3> pos_mask_{1.0, 1.0, 1.0}; //!< mask used for selectively tracking only velocity for single DOF
    };
}
