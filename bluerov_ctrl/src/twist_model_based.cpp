...
TwistModelBasedControl::TwistModelBasedControl(Matrix6d M,
                                               Vector6d damping_linear,
                                               Vector6d damping_nonlinear,
                                               Eigen::Vector3d cog,
                                               Eigen::Vector3d cob, double mass,
...
void TwistModelBasedControl::setVelocityTarget(
...
void TwistModelBasedControl::setAccelerationTarget(
    const double &lin_x, const double &lin_y, const double &lin_z,
    const double &ang_x, const double &ang_y, const double &ang_z) {
...
void TwistModelBasedControl::update(
    const double &dt, const Eigen::Quaterniond &att, const Eigen::Vector3d &vel,
    const Eigen::Vector3d &acc, const Eigen::Vector3d &ang_vel,
  ...
  C_.block<3, 3>(0, 3) =
      -skew(M_.block<3, 3>(0, 0) * vel + M_.block<3, 3>(0, 3) * ang_vel);
  C_.block<3, 3>(3, 0) =
      -skew(M_.block<3, 3>(0, 0) * vel + M_.block<3, 3>(0, 3) * ang_vel);
  C_.block<3, 3>(3, 3) =
      -skew(M_.block<3, 3>(3, 0) * vel + M_.block<3, 3>(3, 3) * ang_vel);

  // update damping matrix
  D_.setZero();
  D_.diagonal().segment<3>(0) =
      damping_linear_.segment<3>(0) +
      (damping_nonlinear_.segment<3>(0).array() * vel.array().abs()).matrix();
  D_.diagonal().segment<3>(3) =
      damping_linear_.segment<3>(3) +
      (damping_nonlinear_.segment<3>(3).array() * vel.array().abs()).matrix();
  ...
  d_component_ = M_ * d_gains_ *
                 (int(d_use_accel_feedforward_) * dnu_des_ -
  ...
  model_component_ =
      int(use_accel_feedforward_) * M_ * dnu_des_ + C_ * nu + D_ * nu + g_;
