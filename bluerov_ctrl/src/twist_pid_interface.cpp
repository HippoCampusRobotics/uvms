...
namespace bluerov_ctrl {
    void TwistPIDInterface::initialize(rclcpp::Node *node_ptr) {
        ...
        accelerations_sub_ = node_ptr_->create_subscription<geometry_msgs::msg::TwistStamped>("accelerations",
                                                                                              ...
                                                                                              std::bind(
                                                                                                      &TwistPIDInterface::onAccelerations,
                                                                                                      this,
                                                                                                      std::placeholders::_1));
    ...
    void TwistPIDInterface::update(const double &dt, const nav_msgs::msg::Odometry &msg,
    ...
    void TwistPIDInterface::setVelocityTarget(const geometry_msgs::msg::Twist &msg) {
        std::lock_guard<std::mutex> lock(mutex_);
        controller_->setVelocityTarget(msg.linear.x, msg.linear.y, msg.linear.z, msg.angular.x,
                                       msg.angular.y, msg.angular.z);
    }

    void TwistPIDInterface::setAccelerationTarget(const geometry_msgs::msg::Twist &msg) {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    void TwistPIDInterface::initializeParamCallbacks() {
        ...
        d_use_accel_feedforward_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&TwistPIDInterface::onDUseAccelFeedforward, this, std::placeholders::_1));
        d_use_accel_estimation_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&TwistPIDInterface::onDUseAccelEstimation, this, std::placeholders::_1));
    }

    void TwistPIDInterface::onAccelerations(const geometry_msgs::msg::TwistStamped::SharedPtr msg) {
    ...
    TwistPIDInterface::onDUseAccelFeedforward(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        bool d_use_accel_feedforward = false;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(mutex_);
            if (hippo_common::param_utils::AssignIfMatch(parameter, "twist_pid.d_use_accel_feedforward",
    ...
    rcl_interfaces::msg::SetParametersResult
    TwistPIDInterface::onDUseAccelEstimation(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        bool d_use_accel_estimation = false;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(mutex_);
            if (hippo_common::param_utils::AssignIfMatch(parameter, "twist_pid.d_use_accel_estimation",
    ...
    rcl_interfaces::msg::SetParametersResult
    TwistPIDInterface::onSetPgains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(mutex_);
            ...
            std::vector<std::string> axes = {"pos_x", "pos_y", "pos_z", "att_x", "att_y", "att_z"};
            for (int i = 0; i < int(axes.size()); i++) {
                if (hippo_common::param_utils::AssignIfMatch(parameter, "twist_pid.gain.p." + axes[i], gain_p_(i))) {
                    controller_->setPgain(gain_p_(i), i);
                    RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
    ...
    rcl_interfaces::msg::SetParametersResult
    TwistPIDInterface::onSetIgains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(mutex_);
            ...
            std::vector<std::string> axes = {"pos_x", "pos_y", "pos_z", "att_x", "att_y", "att_z"};
            for (int i = 0; i < int(axes.size()); i++) {
                if (hippo_common::param_utils::AssignIfMatch(parameter, "twist_pid.gain.i." + axes[i], gain_i_(i))) {
                    controller_->setIgain(gain_i_(i), i);
                    RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
    ...
    rcl_interfaces::msg::SetParametersResult
    TwistPIDInterface::onSetDgains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(mutex_);
            ...
            std::vector<std::string> axes = {"pos_x", "pos_y", "pos_z", "att_x", "att_y", "att_z"};
            for (int i = 0; i < int(axes.size()); i++) {
                if (hippo_common::param_utils::AssignIfMatch(parameter, "twist_pid.gain.d." + axes[i], gain_d_(i))) {
                    controller_->setDgain(gain_d_(i), i);
                    RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
    ...
    void TwistPIDInterface::onTimeout() {
