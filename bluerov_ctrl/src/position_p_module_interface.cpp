...
namespace bluerov_ctrl {
    ...
    void PosPModuleInterface::update(const geometry_msgs::msg::Point &pos, const geometry_msgs::msg::Quaternion &att,
    ...
    void PosPModuleInterface::setControlTarget(const hippo_msgs::msg::ControlTarget::SharedPtr msg) {
        ...
        std::lock_guard <std::mutex> lock(mutex_);
        controller_->setPositionTarget(msg->position.x, msg->position.y, msg->position.z);
        controller_->setVelocityTarget(msg->velocity.x, msg->velocity.y, msg->velocity.z);
    ...
    void PosPModuleInterface::initializeParamCallbacks() {
        p_gains_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&PosPModuleInterface::onSetPgains, this, std::placeholders::_1));
    }

    rcl_interfaces::msg::SetParametersResult
    PosPModuleInterface::onSetPgains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(mutex_);
            if (hippo_common::param_utils::AssignIfMatch(parameter, "pos_p_module.gain.p.x", gain_p_x_)) {
                controller_->setPgainX(gain_p_x_);
                continue;
            }
            if (hippo_common::param_utils::AssignIfMatch(parameter, "pos_p_module.gain.p.y",
                                                         gain_p_y_)) {
                controller_->setPgainY(gain_p_y_);
                continue;
            }
            if (hippo_common::param_utils::AssignIfMatch(parameter, "pos_p_module.gain.p.z", gain_p_z_)) {
                controller_->setPgainZ(gain_p_z_);
                continue;
            }
        }
