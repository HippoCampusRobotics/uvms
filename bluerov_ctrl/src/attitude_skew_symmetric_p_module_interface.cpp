...
namespace bluerov_ctrl {
    ...
    void AttSkewSymmetricPModuleInterface::update(const geometry_msgs::msg::Quaternion &att,
    ...
    void AttSkewSymmetricPModuleInterface::setControlTarget(const hippo_msgs::msg::ControlTarget::SharedPtr msg) {
        ...
        controller_->setAngularVelocityTarget(msg->angular_velocity.x, msg->angular_velocity.y, msg->angular_velocity.z);
    ...
    void AttSkewSymmetricPModuleInterface::initializeParamCallbacks() {
        p_gains_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&AttSkewSymmetricPModuleInterface::onSetPgains, this, std::placeholders::_1));
    }


    rcl_interfaces::msg::SetParametersResult AttSkewSymmetricPModuleInterface::onSetPgains(
            const std::vector <rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard <std::mutex> lock(mutex_);
            if (hippo_common::param_utils::AssignIfMatch(parameter, "att_p_module.gain.roll.p", gain_roll_p_)) {
                controller_->setRollGainP(gain_roll_p_);
                continue;
            }
            if (hippo_common::param_utils::AssignIfMatch(parameter, "att_p_module.gain.pitch.p",
                                                         gain_pitch_p_)) {
                controller_->setPitchGainP(gain_pitch_p_);
                continue;
            }
            if (hippo_common::param_utils::AssignIfMatch(parameter, "att_p_module.gain.yaw.p", gain_yaw_p_)) {
                controller_->setYawGainP(gain_yaw_p_);
                continue;
            }
        }
