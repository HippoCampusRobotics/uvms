...
namespace bluerov_ctrl {
    void TwistModelBasedInterface::initialize(rclcpp::Node *node_ptr) {
        node_ptr_ = node_ptr;
        accelerations_sub_ = node_ptr_->create_subscription<geometry_msgs::msg::TwistStamped>("accelerations",
                                                                                              rclcpp::SystemDefaultsQoS(),
                                                                                              std::bind(
                                                                                                      &TwistModelBasedInterface::onAccelerations,
                                                                                                      this,
                                                                                                      std::placeholders::_1));
        ...
        debug_thrust_p_pub_ = node_ptr_->create_publisher<hippo_msgs::msg::ActuatorSetpoint>("~/thrust_p_component",
                                                                                             rclcpp::SystemDefaultsQoS());
        debug_thrust_i_pub_ = node_ptr_->create_publisher<hippo_msgs::msg::ActuatorSetpoint>("~/thrust_i_component",
                                                                                             rclcpp::SystemDefaultsQoS());
        debug_thrust_d_pub_ = node_ptr_->create_publisher<hippo_msgs::msg::ActuatorSetpoint>("~/thrust_d_component",
                                                                                             rclcpp::SystemDefaultsQoS());
        debug_thrust_model_pub_ = node_ptr_->create_publisher<hippo_msgs::msg::ActuatorSetpoint>(
                "~/thrust_model_component", rclcpp::SystemDefaultsQoS());
        debug_torque_p_pub_ = node_ptr_->create_publisher<hippo_msgs::msg::ActuatorSetpoint>("~/torque_p_component",
                                                                                             rclcpp::SystemDefaultsQoS());
        debug_torque_i_pub_ = node_ptr_->create_publisher<hippo_msgs::msg::ActuatorSetpoint>("~/torque_i_component",
                                                                                             rclcpp::SystemDefaultsQoS());
        debug_torque_d_pub_ = node_ptr_->create_publisher<hippo_msgs::msg::ActuatorSetpoint>("~/torque_d_component",
                                                                                             rclcpp::SystemDefaultsQoS());
        debug_torque_model_pub_ = node_ptr_->create_publisher<hippo_msgs::msg::ActuatorSetpoint>(
    ...
    void TwistModelBasedInterface::initializeController() {
        ...
        descr_text = "Decide if controller acts in body or earth frame (integral term)";
        ...
        controller_ = new TwistModelBasedControl(M, damping_linear, damping_nonlinear,
                                                 cog, cob, mass, buoyancy, body_fixed);
    }


    void TwistModelBasedInterface::update(const double &dt, const nav_msgs::msg::Odometry &msg,
        ...
        hippo_common::convert::RosToEigen(acceleration_msg_.twist.angular, ang_acc_eigen);
        {
            std::lock_guard<std::mutex> lock(mutex_);
            controller_->update(dt, att_eigen, vel_eigen, acc_eigen, ang_vel_eigen, ang_acc_eigen, out_controls);
        }
    ...
    void TwistModelBasedInterface::setVelocityTarget(const geometry_msgs::msg::Twist &msg) {
        std::lock_guard<std::mutex> lock(mutex_);
        controller_->setVelocityTarget(msg.linear.x, msg.linear.y, msg.linear.z, msg.angular.x,
                                       msg.angular.y, msg.angular.z);
    }

    void TwistModelBasedInterface::setAccelerationTarget(const geometry_msgs::msg::Twist &msg) {
        std::lock_guard<std::mutex> lock(mutex_);
        controller_->setAccelerationTarget(msg.linear.x, msg.linear.y, msg.linear.z, msg.angular.x,
                                           msg.angular.y, msg.angular.z);
    }
    ...
    void TwistModelBasedInterface::declareParams() {
        ...
        std::vector<std::string> axes = {"pos_x", "pos_y", "pos_z", "att_x", "att_y", "att_z"};
        ...
        bool use_accel_feedforward;
        {
            ...
            use_accel_feedforward = node_ptr_->declare_parameter(name, use_accel_feedforward, descr);
        ...
        bool d_use_accel_feedforward;
        {
            ...
            d_use_accel_feedforward = node_ptr_->declare_parameter(name, d_use_accel_feedforward, descr);
        ...
        bool d_use_accel_estimation;
        {
            ...
            d_use_accel_estimation = node_ptr_->declare_parameter(name, d_use_accel_estimation, descr);
    ...
    void TwistModelBasedInterface::initializeParamCallbacks() {
        p_gains_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&TwistModelBasedInterface::onSetPgains, this, std::placeholders::_1));
        i_gains_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&TwistModelBasedInterface::onSetIgains, this, std::placeholders::_1));
        d_gains_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&TwistModelBasedInterface::onSetDgains, this, std::placeholders::_1));
        use_accel_feedforward_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&TwistModelBasedInterface::onUseAccelFeedforward, this, std::placeholders::_1));
        d_use_accel_feedforward_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&TwistModelBasedInterface::onDUseAccelFeedforward, this, std::placeholders::_1));
        d_use_accel_estimation_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&TwistModelBasedInterface::onDUseAccelEstimation, this, std::placeholders::_1));
    }


    void TwistModelBasedInterface::onAccelerations(const geometry_msgs::msg::TwistStamped::SharedPtr msg) {
    ...
    void TwistModelBasedInterface::onAccelTimeout() {
        ...
        RCLCPP_WARN(node_ptr_->get_logger(), "Acceleration estimation timed out. Sending zero commands.");
        onTimeout();
    ...
    void TwistModelBasedInterface::publishDebugMsgs() {
        ...
        publishDebugActuator(debug_thrust_model_pub_, debug_torque_model_pub_, component);
    }

    void TwistModelBasedInterface::publishDebugActuator(
            const rclcpp::Publisher<hippo_msgs::msg::ActuatorSetpoint>::SharedPtr &thrust_pub,
    ...
    TwistModelBasedInterface::onUseAccelFeedforward(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        bool use_accel_feedforward = false;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(mutex_);
            if (hippo_common::param_utils::AssignIfMatch(parameter, "model_based.use_accel_feedforward",
                                                         use_accel_feedforward)) {
                controller_->setUseAccelFeedforward(use_accel_feedforward);
                RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                            std::to_string(use_accel_feedforward)).c_str());
                break;
            }
        }
        return result;
    ...
    rcl_interfaces::msg::SetParametersResult
    TwistModelBasedInterface::onDUseAccelFeedforward(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        bool d_use_accel_feedforward = false;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(mutex_);
            if (hippo_common::param_utils::AssignIfMatch(parameter, "model_based.d_use_accel_feedforward",
                                                         d_use_accel_feedforward)) {
                controller_->setDUseAccelFeedforward(d_use_accel_feedforward);
                RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                            std::to_string(d_use_accel_feedforward)).c_str());
                break;
            }
        }
        return result;
    ...
    rcl_interfaces::msg::SetParametersResult
    TwistModelBasedInterface::onDUseAccelEstimation(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        bool d_use_accel_estimation = false;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(mutex_);
            if (hippo_common::param_utils::AssignIfMatch(parameter, "model_based.d_use_accel_estimation",
                                                         d_use_accel_estimation)) {
                controller_->setDUseAccelEstimation(d_use_accel_estimation);
                RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                            std::to_string(d_use_accel_estimation)).c_str());
                break;
            }
        }
        return result;
    ...
    rcl_interfaces::msg::SetParametersResult
    TwistModelBasedInterface::onSetPgains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(mutex_);
            ...
            std::vector<std::string> axes = {"pos_x", "pos_y", "pos_z", "att_x", "att_y", "att_z"};
            for (int i = 0; i < int(axes.size()); i++) {
                if (hippo_common::param_utils::AssignIfMatch(parameter, "model_based.gain.p." + axes[i], gain_p_(i))) {
                    controller_->setPgain(gain_p_(i), i);
                    RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                                std::to_string(gain_p_(i))).c_str());
                    break;
                }
            }
        }
    ...
    rcl_interfaces::msg::SetParametersResult
    TwistModelBasedInterface::onSetIgains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(mutex_);
            ...
            std::vector<std::string> axes = {"pos_x", "pos_y", "pos_z", "att_x", "att_y", "att_z"};
            for (int i = 0; i < int(axes.size()); i++) {
                if (hippo_common::param_utils::AssignIfMatch(parameter, "model_based.gain.i." + axes[i], gain_i_(i))) {
                    controller_->setIgain(gain_i_(i), i);
                    RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                                std::to_string(gain_i_(i))).c_str());
                    break;
                }
            }
        }
    ...
    rcl_interfaces::msg::SetParametersResult
    TwistModelBasedInterface::onSetDgains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(mutex_);
            ...
            std::vector<std::string> axes = {"pos_x", "pos_y", "pos_z", "att_x", "att_y", "att_z"};
            for (int i = 0; i < int(axes.size()); i++) {
                if (hippo_common::param_utils::AssignIfMatch(parameter, "model_based.gain.d." + axes[i], gain_d_(i))) {
                    controller_->setDgain(gain_d_(i), i);
                    RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                                std::to_string(gain_d_(i))).c_str());
                    break;
                }
            }
        }
    ...
    void TwistModelBasedInterface::onTimeout() {
