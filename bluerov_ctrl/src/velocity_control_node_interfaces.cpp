...
void PIDCompInterface::checkStatus() {
  is_ok_ = !velocity_control_interface_->isAccelTimedOut() &&
           (!manipulator_comp_interface_->isActive() ||
            !manipulator_comp_interface_->isTimedOut());
}

void PIDCompInterface::onTimeout() { velocity_control_interface_->onTimeout(); }
