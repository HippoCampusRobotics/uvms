...
namespace bluerov_ctrl {
    ...
    void ManipulatorCompInterface::initialize(rclcpp::Node *node_ptr) {
        ...
        wrench_sub_ = node_ptr->create_subscription<geometry_msgs::msg::WrenchStamped>("force_torque", rclcpp::SystemDefaultsQoS(),
                                                                         std::bind(&ManipulatorCompInterface::onWrench,
                                                                                   this, std::placeholders::_1));

        // initialize debug publishers
        debug_compensation_pub_ = node_ptr_->create_publisher<geometry_msgs::msg::WrenchStamped>("~/compensation_component", rclcpp::SystemDefaultsQoS());
    ...
    void ManipulatorCompInterface::onWrench(const geometry_msgs::msg::WrenchStamped::SharedPtr msg){
        ...
        manipulator_compensation_->setCompensationForce(msg->wrench.force.x, msg->wrench.force.y, msg->wrench.force.z);
        manipulator_compensation_->setCompensationTorque(msg->wrench.torque.x, msg->wrench.torque.y, msg->wrench.torque.z);
    ...
    void ManipulatorCompInterface::addCompensation(const geometry_msgs::msg::Quaternion &msg,
    ...
    void ManipulatorCompInterface::publishDebugMsgs() {
        ...
        debug_msg.header.frame_id = hippo_common::tf2_utils::frame_id::BaseLink(node_ptr_);
    ...
    void ManipulatorCompInterface::declareParams() {
        ...
        std::string descr_text = "Boolean to decide if manipulator compensation is used for control output.";
        rcl_interfaces::msg::ParameterDescriptor descr = hippo_common::param_utils::Description(descr_text);
        active_ = false;
        ...
        for (int i = 0; i < int(suffixes.size()); i++) {
            ...
            buoyancy_origin(i) = node_ptr_->declare_parameter(name, buoyancy_origin(i), descr);
        }

        manipulator_compensation_->setBuoyancyParams(added_buoyancy_mass, buoyancy_origin);
    }

    void ManipulatorCompInterface::initializeParamCallbacks() {
        activation_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&ManipulatorCompInterface::onSetActive, this, std::placeholders::_1));
    }

    rcl_interfaces::msg::SetParametersResult ManipulatorCompInterface::onSetActive(const std::vector<rclcpp::Parameter> &parameters){
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        for (const rclcpp::Parameter &parameter: parameters){
            if (hippo_common::param_utils::AssignIfMatch(parameter, "compensate_manipulator", active_)){
                std::cout << "Set parameter " << parameter.get_name() << " to " << active_ << std::endl;
            }
