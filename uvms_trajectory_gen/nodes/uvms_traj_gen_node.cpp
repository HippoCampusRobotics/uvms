...
namespace uvms_traj_gen {
    UVMSTrajGen::UVMSTrajGen() : Node("traj_gen_node") {
        ...
        ros_param_utils::getParam(this, n_runs_, "n_runs", 5, print_load_motion_output);
        int traj_type;
        ros_param_utils::getParam(this, traj_type, "trajectory_type", 0, print_load_motion_output);
        ...
        eef_traj_pub_ = this->create_publisher<hippo_msgs::msg::ControlTarget>("traj_setpoint", qos);
        if (publish_prediction_) {
            eef_traj_pub_prediction_ = this->create_publisher<uvms_msgs::msg::ControlTargetPrediction>(
                    "traj_setpoint_prediction", qos);
        }
        pose_eef_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>("pose_endeffector", qos,
                                                                                   std::bind(&UVMSTrajGen::updateEef,
                                                                                             this,
                                                                                             std::placeholders::_1));
        angular_vel_debug_pub_ = this->create_publisher<geometry_msgs::msg::Vector3>("~/angular_vel_naive", qos);
        ...
        status_pub_ = this->create_publisher<std_msgs::msg::Int64>("traj_status", qos);
    ...
    void UVMSTrajGen::initializeParameters(bool output) {
        ...
        ros_param_utils::getParam(this, start_accuracy_, "start_accuracy", 0.005, output);
    }

    //!< returns setpoint in world coordinate system, meaning that position, velocity, angular velocity are specified in inertial coordinate system,
    ...
    void UVMSTrajGen::sendSetpoint() {
        ...
        switch (traj_status_) {
            ...
            case TrajStatus::reached_initial_pose: {
                ...
                start_traj_.initializeFromVelocityLimits(pos_, att_, setpoint.pos, setpoint.att, v_max_init_,
                                                         w_max_init_);
                start_time_ = this->now();
            ...
            case TrajStatus::approaching_initial_eef_pose: {
                ...
                Eigen::Vector3d att_error =
                        att_.w() * att_start.vec() - att_start.w() * att_.vec() - att_start_tilde * att_.vec();

                //RCLCPP_INFO(this->get_logger(), "%s", ("Distance to start point: " + std::to_string((pos_start-pos_).norm())).c_str());
                //RCLCPP_INFO(this->get_logger(), "%s", ("Attitude error to start point: " + std::to_string(att_error.norm())).c_str());
                double dt = (this->now() - start_time_).seconds();
                start_traj_.getPositionSetpoint(dt, setpoint.pos, setpoint.vel, setpoint.acc);
                start_traj_.getOrientationSetpoint(dt, setpoint.att, setpoint.ang_vel, setpoint.ang_acc);
                if ((pos_start - pos_).norm() < start_accuracy_ && att_error.norm() < start_accuracy_) {
                    start_time_ = this->now();
                ...
                out_msg_.header.frame_id = hippo_common::tf2_utils::frame_id::kInertialName;
                ...
                hippo_common::convert::EigenToRos(setpoint.ang_vel, out_msg_.angular_velocity);
                hippo_common::convert::EigenToRos(setpoint.ang_acc, out_msg_.angular_acceleration);
                out_msg_.mask = 0;
                ...
                out_msg_prediction_.header.frame_id = hippo_common::tf2_utils::frame_id::kInertialName;
                out_msg_prediction_.target = out_msg_;
                hippo_common::convert::EigenToRos(setpoint.pos, out_msg_prediction_.target_forward.position);
                hippo_common::convert::EigenToRos(setpoint.vel, out_msg_prediction_.target_forward.velocity);
                hippo_common::convert::EigenToRos(setpoint.acc, out_msg_prediction_.target_forward.acceleration);
                hippo_common::convert::EigenToRos(setpoint.att, out_msg_prediction_.target_forward.attitude);
            ...
            case TrajStatus::finished_motion: { // drive back to initial pose
                ...
                run_counter_++;
                RCLCPP_INFO(this->get_logger(), "%s", ("Finished" + std::to_string(run_counter_) + " of " + std::to_string(n_runs_) + " runs.").c_str());
            ...
            case TrajStatus::finished_run: {
                RCLCPP_INFO(this->get_logger(), "Finished run, stopped publishing setpoints");
                return;
            }
            default:
                RCLCPP_ERROR(this->get_logger(), "Unknown trajectory status, no published trajectory.");
                break;
        }
    ...
    void UVMSTrajGen::updateEef(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
