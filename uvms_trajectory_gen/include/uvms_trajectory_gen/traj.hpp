...
namespace uvms_traj_gen {
    ...
    const std::unordered_map<int, std::string> parameter_prefixes = {{TrajectoryType::straight, "straight_line."},
    ...
    public:
        ...
        void addStatusPublisher(rclcpp::Publisher<std_msgs::msg::Int64>::SharedPtr status_pub_ptr){
            status_pub_ptr_ = status_pub_ptr;
        }

        void addStatusPtr(int* status_ptr){
            status_ptr_ = status_ptr;
        }

        virtual bool isFinished(const double &t){
            return false;
        }
    ...
    protected:
        ...
        void getAttitude(const double &t, const Eigen::Vector3d &vel, Eigen::Quaterniond &att, bool upright_z);
        virtual void getTranslationalMotion(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel) = 0;
        virtual void getTangential(const double &t, const Eigen::Vector3d &vel, Eigen::Vector3d &tangential);
        virtual void getTangentialRecursive(const double &t, const Eigen::Vector3d &vel, const double &initial_vel_norm, int &counter, Eigen::Vector3d &tangential);
    ...
    public:
        ...
        bool isFinished(const double &t) override{
            if (unidirectional_ && t >= start_trajectory_.getDuration() + duration_ + end_trajectory_.getDuration()){
        ...
        int trajType() override{
            return type_;
        }


    private:
        void getTranslationalMotion(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel) override;
        void getStraightLineSetpoint(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel);
        void getStraightLineSetpoint(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel, Eigen::Vector3d &acc);
    ...
    private:
        ...
        //!< Returns position, velocity and acceleration all specified in inertial coordinate system
        void getTranslationalMotion(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel) override;
        ...
        double period_;
        double amp_;
        double w_oscillations_; //!< frequency of motions perpendicular to movement direction
    ...
    public:
        ...
        bool isFinished(const double &t) override{
            if (unidirectional_ && t >= T_ / 2 + start_trajectory_.getDuration() + end_trajectory_.getDuration()){
                got_last_tangential_ = false;
                return true;
            }
            return false;
    ...
    private:
        ...
        void getSiSetpoint(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel);
        void getSiFunc(const double &t, const double &delta_t, double &x, double &dx, double &ddx);
        void getSiSetpoint(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel, Eigen::Vector3d &acc);
        void getTranslationalMotion(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel) override;
    ...
    public:
        ...
        bool isFinished(const double &t) override{
            if (unidirectional_ && t >= T_ + start_trajectory_.getDuration() + end_trajectory_.getDuration()){
                got_last_tangential_ = false;
                return true;
            } else if (t / (T_ + start_trajectory_.getDuration() + end_trajectory_.getDuration()) >= double(n_turns_)){
              got_last_tangential_ = false;
              return true;
            }
    ...
    private:
        ...
        //!< Returns position, velocity and acceleration all specified in inertial coordinate system
        void getTranslationalMotion(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel) override;
        void getFlowerSetpoint(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel);
        void getFlowerSetpoint(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel, Eigen::Vector3d &acc);
    ...
    private:
        //!< Returns position, velocity and acceleration all specified in inertial coordinate system
        void getTranslationalMotion(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel) override;
        void getSinusSetpoint(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel);
        void getSinusSetpoint(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel, Eigen::Vector3d &acc);
    ...
    private:
        //!< Returns position, velocity and acceleration all specified in inertial coordinate system
        void getTranslationalMotion(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel) override;
        void getSpiralSetpoint(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel);
        void getSpiralSetpoint(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel, Eigen::Vector3d &acc);
    ...
    public:
        ...
        bool isFinished(const double &t) override{
            if (unidirectional_ && t >= start_trajectory_.getDuration() + T_ + end_trajectory_.getDuration()){
                got_last_tangential_ = false;
                return true;
            }
            return false;
    ...
    private:
        //!< Returns position, velocity and acceleration all specified in inertial coordinate system
        void getTranslationalMotion(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel) override;
        void getEightSetpoint(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel);
        void getEightSetpoint(const double &t, Eigen::Vector3d &pos, Eigen::Vector3d &vel, Eigen::Vector3d &acc);
