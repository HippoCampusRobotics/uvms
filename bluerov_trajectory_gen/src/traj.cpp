...
namespace bluerov_traj_gen {
    ...
    void SingleDOFVelTraj::initialize(rclcpp::Node* node_ptr, bool output){
        ...
        if (!ros_param_utils::getParamArray(node_ptr_, start_pos, name_prefix + prefix + "start_pos", {1.0, 1.5, -0.5})) {
        ...
        if (!ros_param_utils::getParamArray(node_ptr_, start_att, name_prefix + prefix + "start_att", {0.0, 0.0, 0.0})) {
        ...
        att_des_ = Eigen::AngleAxisd(start_att[2] * M_PI, Eigen::Vector3d::UnitZ()) * Eigen::AngleAxisd(start_att[1] * M_PI, Eigen::Vector3d::UnitY()) *
                Eigen::AngleAxisd(start_att[0] * M_PI, Eigen::Vector3d::UnitX());

        std::vector<double> limits;
        if (!ros_param_utils::getParamArray(node_ptr_, limits, name_prefix + prefix + "limits", {0.0, 0.0})) {
        ...
        if (!ros_param_utils::getParam(node_ptr_, vel_, name_prefix + prefix + "vel", 4.0)) {
    ...
    void SingleDOFVelTraj::getSetpoint(const double &t, TrajSetpoint &out){
        ...
        if (idx_ < 2){ // x- and y-direction of BlueROV are both tested along global y-axis
            ...
            if (rpy(idx_-3) >= limits_[1]){
                ...
                out.att.setIdentity();
                for (int i = 2; i >= 0; i--){
                    out.att = out.att * Eigen::AngleAxis(int(i == idx_-3) * limits_[1] + int(i != idx_-3) * rpy_des(i), Eigen::Vector3d::Unit(i));
                }
                ...
                out.att.setIdentity();
                for (int i = 2; i >= 0; i--){
                    out.att = out.att * Eigen::AngleAxis(int(i == idx_-3) * limits_[0] + int(i != idx_-3) * rpy_des(i), Eigen::Vector3d::Unit(i));

                }
                out.ang_vel(idx_ - 3) = 0.0;
            }
        } else {
            std::cerr << "Velocity index " << idx_ << " of trajectory out of range!" << std::endl;
            return;
        }
    ...
    void SingleDOFVelTraj::getSetpointMsg(const double &t, hippo_msgs::msg::ControlTarget &out) {
        TrajSetpoint setpoint;
        ...
        switch (idx_){
            ...
            case 5:
                ...
                break;
            default:
                std::cerr << "Index " << idx_ << " is out of range for ignoring state" << std::endl;
        }
    }
    ...
    void SingleDOFSetpoints::initialize(rclcpp::Node* node_ptr, bool output){
        ...
        if (!ros_param_utils::getParam(node_ptr_, n_setpoints_, name_prefix + "n_setpoints", 5)) {
        ...
        std::generate(setpoints_.begin(), setpoints_.end(), [n = 0, &interval, &min]() mutable { return n++ * interval + min; });

        if (!ros_param_utils::getParam(node_ptr_, t_setpoint_, name_prefix + "t_setpoint", 5.0)) {
            RCLCPP_ERROR(node_ptr_->get_logger(), "Param n_setpoints not set!");
            return;
        }
        if (!ros_param_utils::getParam(node_ptr_, accuracy_, name_prefix + "accuracy", 0.01)) {
    ...
    void SingleDOFSetpoints::getSetpoint(const double &t, TrajSetpoint &out){
        ...
        Eigen::Vector3d rpy;
        rot_utils::quat_to_rpy(*att_ptr_, rpy);
        if (!keep_pose_ && std::abs(setpoints_[idx_setpoint_] - rpy(idx_)) < accuracy_){
            t_reached_ = t;
            keep_pose_ = true;
        ...
        out.att = Eigen::Quaterniond(Eigen::AngleAxisd(setpoints_[idx_setpoint_], Eigen::Vector3d::Unit(idx_)));
        //out.ang_vel(idx_) = setpoints_[idx_setpoint_] - rpy(idx_);
    }

    void SingleDOFSetpoints::getSetpointMsg(const double &t, hippo_msgs::msg::ControlTarget &out) {
        TrajSetpoint setpoint;
        getSetpoint(t, setpoint);
        out.header.stamp = node_ptr_->now();
        out.header.frame_id = hippo_common::tf2_utils::frame_id::InertialFrame();
    ...
    void SingleDOFSinusoidal::initialize(rclcpp::Node *node_ptr, bool output) {
        ...
        if (!ros_param_utils::getParam(node_ptr_, amplitude_, name_prefix + prefix + "amplitude", 0.1)) {
        ...
        if (!ros_param_utils::getParam(node_ptr_, omega_, name_prefix + prefix + "omega", 0.1)) {
    ...
    void SingleDOFSinusoidal::getSetpoint(const double &t, TrajSetpoint &out) {
        ...
        if (idx_ < 2){ // x- and y-direction of BlueROV are both tested along global y-axis
            ...
            Eigen::Vector3d rpy;
            rot_utils::quat_to_rpy(*att_ptr_, rpy);
            out.att = out.att * Eigen::AngleAxis(amplitude_ * sin(omega_ * t), Eigen::Vector3d::Unit(idx_ - 3));
            out.ang_vel(idx_ - 3) = amplitude_ * omega_ * cos(omega_ * t);
            out.ang_acc(idx_ - 3) = - amplitude_ * std::pow(omega_, 2) * sin(omega_ * t);
        } else {
            std::cerr << "Index " << idx_ << " of trajectory out of range!" << std::endl;
            return;
        }
    }

    void SingleDOFSinusoidal::getSetpointMsg(const double &t, hippo_msgs::msg::ControlTarget &out) {
    ...
    void StationKeeping::initialize(rclcpp::Node* node_ptr, bool output){
        ...
        std::vector<double> start_pos;
        if (!ros_param_utils::getParamArray(node_ptr_, start_pos, name_prefix + "start_pos", {1.0, 1.5, -0.5})) {
            RCLCPP_ERROR(node_ptr_->get_logger(), "Param start_pos not set!");
            return;
        }
        if (start_pos.size() != 3){
        ...
        pos_des_ = Eigen::Vector3d(start_pos.data());
        std::vector<double> start_att;
        if (!ros_param_utils::getParamArray(node_ptr_, start_att, name_prefix + "start_att", {0.0, 0.0, 0.0})) {
            RCLCPP_ERROR(node_ptr_->get_logger(), "Param start_att not set!");
            return;
        }
        if (start_att.size() != 3){
    ...
    void StationKeeping::getSetpointMsg(const double &t, hippo_msgs::msg::ControlTarget &out) {
        TrajSetpoint setpoint;
        getSetpoint(t, setpoint);
        out.header.stamp = node_ptr_->now();
        out.header.frame_id = hippo_common::tf2_utils::frame_id::InertialFrame();
    ...
    void EightTraj::initialize(rclcpp::Node* node_ptr, bool output){
        ...
        if (!ros_param_utils::getParam(node_ptr_, x_width_, name_prefix + "x_width", 2.0)) {
            RCLCPP_ERROR(node_ptr_->get_logger(), "Param tank x-width not set!");
            return;
        }

        if (!ros_param_utils::getParam(node_ptr_, y_width_, name_prefix + "y_width", 4.0)) {
            RCLCPP_ERROR(node_ptr_->get_logger(), "Param tank y-width not set!");
            return;
        }

        if (!ros_param_utils::getParam(node_ptr_, height_, name_prefix + "height", -0.5)) {
            RCLCPP_ERROR(node_ptr_->get_logger(), "Param desired height not set!");
            return;
        }


        if (!ros_param_utils::getParam(node_ptr_, max_roll_, name_prefix + "max_roll", 0.0)) {
            RCLCPP_ERROR(node_ptr_->get_logger(), "Param accuracy for positioning not set!");
            return;
        }
    ...
    void EightTraj::getSetpointMsg(const double &t, hippo_msgs::msg::ControlTarget &out) {
        TrajSetpoint setpoint;
        getSetpoint(t, setpoint);
        out.header.stamp = node_ptr_->now();
        out.header.frame_id = hippo_common::tf2_utils::frame_id::InertialFrame();
    ...
    void EightTraj::getEightVel(Eigen::Vector3d &vel, const double &t) {
        ...
        vel(1) = a_ * omega_ * sin(omega_ * t) * (std::pow(sin(omega_ * t), 2) - 3) / denum2;
        vel(2) = 0.0;
    }
    ...
    void EightTraj::getEightRPY(double &roll, double &pitch, double &yaw, const double &t){
        getEightYaw(yaw, t);
    ...
    void EightTraj::getEightRates(Eigen::Vector3d &rates, const double &t) {
        ...
        deuler_param <<  -omega_ * max_roll_ * sin(omega_ * t),  0.0, (yaw_next - yaw_prev) / (2 * eps);
