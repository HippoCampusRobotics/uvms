...
namespace bluerov_traj_gen {
    ...
    public:
        MotionPlanner() : Node("trajectory_gen") {
            initialize();
        };
    private:
        void initialize() {
            ...
            int traj_type;
            ros_param_utils::getParam(this, traj_type, "trajectory_type", 1, print_load_motion_output);
            ...
            switch (traj_type){
                ...
                case (TrajectoryType::single_DOF):
                    ...
                    dynamic_cast<SingleDOFVelTraj*>(traj_gen_)->setStartedLoopFlagPtr(&start_loop_);
                    dynamic_cast<SingleDOFVelTraj*>(traj_gen_)->setGenStartTrajFlagPtr(&gen_start_traj_);
                    break;
            ...
            setpoint_pub_ = this->create_publisher<hippo_msgs::msg::ControlTarget>("traj_setpoint", rclcpp::SystemDefaultsQoS());
            state_sub_ = this->create_subscription<nav_msgs::msg::Odometry>("odometry", rclcpp::SystemDefaultsQoS(),
                                                                            std::bind(&MotionPlanner::update_state,
                                                                                      this, _1));
            wait_for_start_ = false;
            start_loop_ = false;
            gen_start_traj_ = true;
            double freq = 50;
            timer_ = rclcpp::create_timer(this, this->get_clock(), static_cast<std::chrono::milliseconds>(int(1000*1.0 / freq)),
                                             std::bind(&MotionPlanner::sendSetpoint, this));
            status_pub_ = this->create_publisher<std_msgs::msg::Int64>("traj_status", rclcpp::SystemDefaultsQoS());
        ...
        void sendSetpoint() {
            ...
            if (gen_start_traj_){
                ...
                start_traj_.initializeFromVelocityLimits(pos_, att_, pos_start, att_start, v_max_init_, w_max_init_);
            ...
            if (!start_loop_) {
                ...
                Eigen::Vector3d att_error = att_.w() * att_start.vec() - att_start.w() * att_.vec() - att_start_tilde * att_.vec();
                ...
                out_msg_.header.frame_id = hippo_common::tf2_utils::frame_id::InertialFrame();
                ...
                hippo_common::convert::EigenToRos(ang_acc_des, out_msg_.angular_acceleration);
                out_msg_.mask = 0;
