...
namespace visualization{
    ...
    public:
        VisualizationNode(): Node("visualization_node"){
            ...
            timer_ = rclcpp::create_timer(this, this->get_clock(), 50ms, std::bind(&VisualizationNode::publishMarkers, this));
            marker_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("viz_marker", qos);

        }

        void initializeModules(){
            std::vector<int64_t> visualization_modules;
            ros_param_utils::getParamArray(this, visualization_modules, "visualization_modules", {1});
            for (auto& visualization_module : visualization_modules){
                switch (visualization_module){
                    case uvms_visualization::visualization_modules::endeffector:
                        modules_.emplace_back(std::make_unique<uvms_visualization::EndeffectorVisualization>());
                        break;
                    case uvms_visualization::visualization_modules::target:
                        modules_.emplace_back(std::make_unique<uvms_visualization::TargetVisualization>());
                        break;
                    case uvms_visualization::visualization_modules::velocity_target:
                        modules_.emplace_back(std::make_unique<uvms_visualization::VelocityTargetVisualization>());
                        break;
                    case uvms_visualization::visualization_modules::auv_velocity_target:
                        modules_.emplace_back(std::make_unique<uvms_visualization::AUVVelocityTargetVisualization>());
                        break;
                    case uvms_visualization::visualization_modules::auv_thrust:
                        modules_.emplace_back(std::make_unique<uvms_visualization::AUVThrustVisualization>());
                        break;
                    case uvms_visualization::visualization_modules::auv_pose:
                        modules_.emplace_back(std::make_unique<uvms_visualization::AUVPoseVisualization>());
                        break;
                    case uvms_visualization::visualization_modules::uvms_target:
                        modules_.emplace_back(std::make_unique<uvms_visualization::UVMSTargetVisualization>());
                        break;
                    case uvms_visualization::visualization_modules::endeffector_frame:
                      modules_.emplace_back(std::make_unique<uvms_visualization::EndeffectorFrameVisualization>());
                      break;
                    case uvms_visualization::visualization_modules::target_frame:
                      modules_.emplace_back(std::make_unique<uvms_visualization::TargetFrameVisualization>());
                      break;
                    case uvms_visualization::visualization_modules::endeffector_axis:
                      modules_.emplace_back(std::make_unique<uvms_visualization::EndeffectorAxisVisualization>());
                      break;
                    default:
                        RCLCPP_ERROR(this->get_logger(), "Unknown visualization module, aborting initialization");
                        return;
                }
            }
    ...
    private:
        void publishMarkers(){
            marker_pub_->publish(*marker_.get());
        }
