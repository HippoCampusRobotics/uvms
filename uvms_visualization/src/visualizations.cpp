...
namespace uvms_visualization {
    ...
    void EndeffectorVisualization::initialize(rclcpp::Node* node_ptr, std::shared_ptr<visualization_msgs::msg::MarkerArray> marker_ptr) {
        node_ptr_ = node_ptr;
        marker_ptr_ = marker_ptr;
        rclcpp::QoS qos = rclcpp::SystemDefaultsQoS();
        eef_sub_ = node_ptr_->create_subscription<geometry_msgs::msg::PoseStamped>("pose_endeffector", qos,
                                                                              std::bind(&EndeffectorVisualization::onEndeffectorPose,
    ...
    void EndeffectorVisualization::onEndeffectorPose(const geometry_msgs::msg::PoseStamped::SharedPtr msg){

        marker_ptr_->markers[idx_map_.at("pos_est_eef")].header.stamp = msg->header.stamp;
        if (int(marker_ptr_->markers[idx_map_.at("pos_est_eef")].points.size()) < kNMarker) {
            marker_ptr_->markers[idx_map_.at("pos_est_eef")].points.push_back(msg->pose.position);
        } else {
            marker_ptr_->markers[idx_map_.at("pos_est_eef")].points.push_back(msg->pose.position);
            for (int i = 0; i < kNMarker; i++) {
                marker_ptr_->markers[idx_map_.at("pos_est_eef")].points.at(i) = marker_ptr_->markers[idx_map_.at("pos_est_eef")].points.at(i + 1);
            }
    ...
    void EndeffectorFrameVisualization::initialize(rclcpp::Node* node_ptr, std::shared_ptr<visualization_msgs::msg::MarkerArray> marker_ptr) {
        node_ptr_ = node_ptr;
        marker_ptr_ = marker_ptr;
        rclcpp::QoS qos = rclcpp::SystemDefaultsQoS();
        eef_sub_ = node_ptr_->create_subscription<geometry_msgs::msg::PoseStamped>("pose_endeffector", qos,
                                                                              std::bind(&EndeffectorFrameVisualization::onEndeffectorPose,
                                                                                        this, std::placeholders::_1));
    }
    ...
    void EndeffectorFrameVisualization::initializeMarkers() {
        ...
        marker_est_pose_x.header.frame_id = hippo_common::tf2_utils::frame_id::kInertialName;
        ...
        marker_est_pose_y.header.frame_id = hippo_common::tf2_utils::frame_id::kInertialName;
        ...
        marker_est_pose_z.header.frame_id = hippo_common::tf2_utils::frame_id::kInertialName;
    ...
    void EndeffectorFrameVisualization::onEndeffectorPose(const geometry_msgs::msg::PoseStamped::SharedPtr msg){


        marker_ptr_->markers[idx_map_.at("pose_est_eef_x")].header.stamp = msg->header.stamp;
        if (!marker_ptr_->markers[idx_map_.at("pose_est_eef_x")].points.empty()) {
            // set pose
            marker_ptr_->markers[idx_map_.at("pose_est_eef_x")].pose.position = msg->pose.position;
            marker_ptr_->markers[idx_map_.at("pose_est_eef_x")].pose.orientation = msg->pose.orientation;
        } else {
            geometry_msgs::msg::Point p;
            p.x = 0.0;
            p.y = 0.0;
        ...
        marker_ptr_->markers[idx_map_.at("pose_est_eef_y")].header.stamp = msg->header.stamp;
        if (!marker_ptr_->markers[idx_map_.at("pose_est_eef_y")].points.empty()) {
            // set pose
            marker_ptr_->markers[idx_map_.at("pose_est_eef_y")].pose.position = msg->pose.position;
            marker_ptr_->markers[idx_map_.at("pose_est_eef_y")].pose.orientation = msg->pose.orientation;
        } else {
            geometry_msgs::msg::Point p;
            p.x = 0.0;
            p.y = 0.0;
        ...
        marker_ptr_->markers[idx_map_.at("pose_est_eef_z")].header.stamp = msg->header.stamp;
        if (!marker_ptr_->markers[idx_map_.at("pose_est_eef_z")].points.empty()) {

            // set pose
            marker_ptr_->markers[idx_map_.at("pose_est_eef_z")].pose.position = msg->pose.position;
            marker_ptr_->markers[idx_map_.at("pose_est_eef_z")].pose.orientation = msg->pose.orientation;
        } else {
            geometry_msgs::msg::Point p;
            p.x = 0.0;
            p.y = 0.0;
    ...
    void EndeffectorAxisVisualization::initialize(rclcpp::Node* node_ptr, std::shared_ptr<visualization_msgs::msg::MarkerArray> marker_ptr) {
      node_ptr_ = node_ptr;
      marker_ptr_ = marker_ptr;
      rclcpp::QoS qos = rclcpp::SystemDefaultsQoS();
      eef_sub_ = node_ptr_->create_subscription<geometry_msgs::msg::PoseStamped>("pose_endeffector", qos,
                                                                                 std::bind(&EndeffectorAxisVisualization::onEndeffectorPose,
                                                                                           this, std::placeholders::_1));
    }
    ...
    void EndeffectorAxisVisualization::onEndeffectorPose(const geometry_msgs::msg::PoseStamped::SharedPtr msg){
    ...
    void TargetVisualization::initialize(rclcpp::Node* node_ptr,
                                         visualization_msgs::msg::MarkerArray::SharedPtr marker_ptr) {
        node_ptr_ = node_ptr;
        marker_ptr_ = marker_ptr;
        rclcpp::QoS qos = rclcpp::SystemDefaultsQoS();
        target_sub_ = node_ptr_->create_subscription<hippo_msgs::msg::ControlTarget>("traj_setpoint", qos, std::bind(
                &TargetVisualization::onTarget, this, std::placeholders::_1));
    }
    ...
    void TargetVisualization::onTarget(const hippo_msgs::msg::ControlTarget::SharedPtr msg) {
        geometry_msgs::msg::Point p;
        if (msg->mask != msg->IGNORE_POSITION && msg->mask != msg->IGNORE_POSE) {
            marker_ptr_->markers[idx_map_.at("pos_des")].header.stamp = msg->header.stamp;
            p.x = msg->position.x;
            p.y = msg->position.y;
            p.z = msg->position.z;
            if (int(marker_ptr_->markers[idx_map_.at("pos_des")].points.size()) < kNMarker) {
    ...
    void TargetFrameVisualization::initialize(rclcpp::Node* node_ptr,
                                         visualization_msgs::msg::MarkerArray::SharedPtr marker_ptr) {
        node_ptr_ = node_ptr;
        marker_ptr_ = marker_ptr;
        rclcpp::QoS qos = rclcpp::SystemDefaultsQoS();
        target_sub_ = node_ptr_->create_subscription<hippo_msgs::msg::ControlTarget>("traj_setpoint", qos, std::bind(
                &TargetFrameVisualization::onTarget, this, std::placeholders::_1));
    }
    ...
    void TargetFrameVisualization::initializeMarkers() {
        ...
        marker_des_pose_x.header.frame_id = hippo_common::tf2_utils::frame_id::kInertialName;
        ...
        marker_des_pose_y.header.frame_id = hippo_common::tf2_utils::frame_id::kInertialName;
        ...
        marker_des_pose_z.header.frame_id = hippo_common::tf2_utils::frame_id::kInertialName;
    ...
    void TargetFrameVisualization::onTarget(const hippo_msgs::msg::ControlTarget::SharedPtr msg) {
        geometry_msgs::msg::Point p;

        if (msg->mask != msg->IGNORE_POSE) {
            marker_ptr_->markers[idx_map_.at("pose_des_x")].header.stamp = msg->header.stamp;
            if (!marker_ptr_->markers[idx_map_.at("pose_des_x")].points.empty()) {
                // set pose
                marker_ptr_->markers[idx_map_.at("pose_des_x")].pose.position = msg->position;
                marker_ptr_->markers[idx_map_.at("pose_des_x")].pose.orientation = msg->attitude;
            } else {
                p.x = 0.0;
                p.y = 0.0;
                p.z = 0.0;
            ...
            marker_ptr_->markers[idx_map_.at("pose_des_y")].header.stamp = msg->header.stamp;
            if (!marker_ptr_->markers[idx_map_.at("pose_des_y")].points.empty()) {
                // set pose
                marker_ptr_->markers[idx_map_.at("pose_des_y")].pose.position = msg->position;
                marker_ptr_->markers[idx_map_.at("pose_des_y")].pose.orientation = msg->attitude;
            } else {
                p.x = 0.0;
                p.y = 0.0;
                p.z = 0.0;
            ...
            marker_ptr_->markers[idx_map_.at("pose_des_z")].header.stamp = msg->header.stamp;
            if (!marker_ptr_->markers[idx_map_.at("pose_des_z")].points.empty()) {

                // set pose
                marker_ptr_->markers[idx_map_.at("pose_des_z")].pose.position = msg->position;
                marker_ptr_->markers[idx_map_.at("pose_des_z")].pose.orientation = msg->attitude;
            } else {
                p.x = 0.0;
                p.y = 0.0;
                p.z = 0.0;
    ...
    void VelocityTargetVisualization::initialize(rclcpp::Node* node_ptr,
                                         visualization_msgs::msg::MarkerArray::SharedPtr marker_ptr) {
        node_ptr_ = node_ptr;
        marker_ptr_ = marker_ptr;
        rclcpp::QoS qos = rclcpp::SystemDefaultsQoS();
        target_sub_ = node_ptr_->create_subscription<hippo_msgs::msg::ControlTarget>("traj_setpoint", qos, std::bind(
                &VelocityTargetVisualization::onTarget, this, std::placeholders::_1));
    }
    ...
    void VelocityTargetVisualization::initializeMarkers() {
        ...
        marker_des_dir.header.frame_id = hippo_common::tf2_utils::frame_id::kInertialName;
    ...
    void VelocityTargetVisualization::onTarget(const hippo_msgs::msg::ControlTarget::SharedPtr msg) {
        double norm = sqrt(std::pow(msg->velocity.x, 2) + std::pow(msg->velocity.y, 2) + std::pow(msg->velocity.z, 2));
        if (norm >= 0.001) {
            const double length = 0.5;
            marker_ptr_->markers[idx_map_.at("vel_dir_des")].header.stamp = msg->header.stamp;
            if (!marker_ptr_->markers[idx_map_.at("vel_dir_des")].points.empty()) {
                marker_ptr_->markers[idx_map_.at("vel_dir_des")].points.at(0).x = msg->position.x;
                marker_ptr_->markers[idx_map_.at("vel_dir_des")].points.at(0).y = msg->position.y;
                marker_ptr_->markers[idx_map_.at("vel_dir_des")].points.at(0).z = msg->position.z;
                marker_ptr_->markers[idx_map_.at("vel_dir_des")].points.at(1).x = length * msg->velocity.x / norm + msg->position.x;
                marker_ptr_->markers[idx_map_.at("vel_dir_des")].points.at(1).y = length * msg->velocity.y / norm + msg->position.y;
                marker_ptr_->markers[idx_map_.at("vel_dir_des")].points.at(1).z = length * msg->velocity.z / norm + msg->position.z;
            } else {
                geometry_msgs::msg::Point p;
                p.x = msg->position.x;
                p.y = msg->position.y;
    ...
    void AUVPoseVisualization::initialize(rclcpp::Node* node_ptr, std::shared_ptr<visualization_msgs::msg::MarkerArray> marker_ptr) {
        node_ptr_ = node_ptr;
        marker_ptr_ = marker_ptr;
        rclcpp::QoS qos = rclcpp::SystemDefaultsQoS();
        odom_sub_ = node_ptr_->create_subscription<nav_msgs::msg::Odometry>("odometry", qos,
                                                                                   std::bind(&AUVPoseVisualization::onOdometry,
                                                                                             this, std::placeholders::_1));
    }
    ...
    void AUVPoseVisualization::onOdometry(const nav_msgs::msg::Odometry::SharedPtr msg){

        marker_ptr_->markers[idx_map_.at("pos_est")].header.stamp = msg->header.stamp;
        if (int(marker_ptr_->markers[idx_map_.at("pos_est")].points.size()) < kNMarker) {
            marker_ptr_->markers[idx_map_.at("pos_est")].points.push_back(msg->pose.pose.position);
        } else {
            marker_ptr_->markers[idx_map_.at("pos_est")].points.push_back(msg->pose.pose.position);
            for (int i = 0; i < kNMarker; i++) {
                marker_ptr_->markers[idx_map_.at("pos_est")].points.at(i) = marker_ptr_->markers[idx_map_.at("pos_est")].points.at(i + 1);
            }
        ...
        marker_ptr_->markers[idx_map_.at("pose_est_x")].header.stamp = msg->header.stamp;
        if (!marker_ptr_->markers[idx_map_.at("pose_est_x")].points.empty()) {
            // set pose
            marker_ptr_->markers[idx_map_.at("pose_est_x")].pose.position = msg->pose.pose.position;
            marker_ptr_->markers[idx_map_.at("pose_est_x")].pose.orientation = msg->pose.pose.orientation;
        } else {
            geometry_msgs::msg::Point p;
            p.x = 0.0;
            p.y = 0.0;
        ...
        marker_ptr_->markers[idx_map_.at("pose_est_y")].header.stamp = msg->header.stamp;
        if (!marker_ptr_->markers[idx_map_.at("pose_est_y")].points.empty()) {
            // set pose
            marker_ptr_->markers[idx_map_.at("pose_est_y")].pose.position = msg->pose.pose.position;
            marker_ptr_->markers[idx_map_.at("pose_est_y")].pose.orientation = msg->pose.pose.orientation;
        } else {
            geometry_msgs::msg::Point p;
            p.x = 0.0;
            p.y = 0.0;
        ...
        marker_ptr_->markers[idx_map_.at("pose_est_z")].header.stamp = msg->header.stamp;
        if (!marker_ptr_->markers[idx_map_.at("pose_est_z")].points.empty()) {

            // set pose
            marker_ptr_->markers[idx_map_.at("pose_est_z")].pose.position = msg->pose.pose.position;
            marker_ptr_->markers[idx_map_.at("pose_est_z")].pose.orientation = msg->pose.pose.orientation;
        } else {
            geometry_msgs::msg::Point p;
            p.x = 0.0;
            p.y = 0.0;
    ...
    void AUVVelocityTargetVisualization::initialize(rclcpp::Node* node_ptr,
                                                 visualization_msgs::msg::MarkerArray::SharedPtr marker_ptr) {
        node_ptr_ = node_ptr;
        marker_ptr_ = marker_ptr;
        rclcpp::QoS qos = rclcpp::SystemDefaultsQoS();
        velocity_target_sub_ = node_ptr_->create_subscription<hippo_msgs::msg::VelocityControlTarget>("velocity_setpoint", qos, std::bind(
                &AUVVelocityTargetVisualization::onVelocityTarget, this, std::placeholders::_1));
    }
    ...
    void AUVVelocityTargetVisualization::initializeMarkers() {
        ...
        marker_des_dir.type = visualization_msgs::msg::Marker::ARROW;
        marker_des_dir.action = visualization_msgs::msg::Marker::ADD;
        marker_des_dir.header.frame_id = hippo_common::tf2_utils::frame_id::BaseLink(node_ptr_);
        marker_des_dir.id = idx;
        marker_des_dir.color.a = 0.7;
    ...
    void AUVVelocityTargetVisualization::onVelocityTarget(const hippo_msgs::msg::VelocityControlTarget::SharedPtr msg) {
        double norm = sqrt(std::pow(msg->velocity.linear.x, 2) + std::pow(msg->velocity.linear.y, 2) + std::pow(msg->velocity.linear.z, 2));
        if (norm >= 0.001) {
            marker_ptr_->markers[idx_map_.at("vel_target")].header.stamp = msg->header.stamp;
            if (!marker_ptr_->markers[idx_map_.at("vel_target")].points.empty()) {
                ...
                marker_ptr_->markers[idx_map_.at("vel_target")].points.at(1).x = msg->velocity.linear.x / norm;
                marker_ptr_->markers[idx_map_.at("vel_target")].points.at(1).y = msg->velocity.linear.y / norm;
                marker_ptr_->markers[idx_map_.at("vel_target")].points.at(1).z = msg->velocity.linear.z / norm;
            } else {
                geometry_msgs::msg::Point p;
                p.x = 0.0;
                p.y = 0.0;
    ...
    void AUVThrustVisualization::initialize(rclcpp::Node* node_ptr,
                                            visualization_msgs::msg::MarkerArray::SharedPtr marker_ptr) {
        node_ptr_ = node_ptr;
        marker_ptr_ = marker_ptr;
        rclcpp::QoS qos = rclcpp::SystemDefaultsQoS();
        thrust_sub_ = node_ptr_->create_subscription<hippo_msgs::msg::ActuatorSetpoint>("thrust_setpoint", qos, std::bind(
                &AUVThrustVisualization::onThrust, this, std::placeholders::_1));
    }
    ...
    void AUVThrustVisualization::initializeMarkers() {
        ...
        marker_thrust.header.frame_id = hippo_common::tf2_utils::frame_id::BaseLink(node_ptr_);
    ...
    void AUVThrustVisualization::onThrust(const hippo_msgs::msg::ActuatorSetpoint::SharedPtr msg) {
        marker_ptr_->markers[idx_map_.at("thrust_dir")].header.stamp = msg->header.stamp;
    ...
    void UVMSTargetVisualization::initialize(rclcpp::Node *node_ptr,
                                             visualization_msgs::msg::MarkerArray::SharedPtr marker_ptr) {
        node_ptr_ = node_ptr;
        marker_ptr_ = marker_ptr;
        rclcpp::QoS qos = rclcpp::SystemDefaultsQoS();
        uvms_target_sub_ = node_ptr_->create_subscription<uvms_msgs::msg::UVMSControlTarget>("traj_setpoint_uvms", qos, std::bind(
                &UVMSTargetVisualization::onUVMSTarget, this, std::placeholders::_1));
    }

    void UVMSTargetVisualization::onUVMSTarget(const uvms_msgs::msg::UVMSControlTarget::SharedPtr msg) {
        hippo_msgs::msg::ControlTarget auv_msg;
        auv_msg = msg->auv;
        auv_msg.header = msg->header;
        hippo_msgs::msg::ControlTarget::SharedPtr auv_msg_ptr = std::make_shared<hippo_msgs::msg::ControlTarget>(auv_msg);
