...
def generate_launch_description():
    ...
    visualization_topics = [
        "/pose_endeffector", "/traj_setpoint", "/odometry",
        "/velocity_setpoint", "/thrust_setpoint", "/traj_setpoint_uvms"
    ]
    print([("/" + namespace + topic, vehicle_namespace + topic)
           for topic in visualization_topics])
    return launch.LaunchDescription([
        use_sim_time_launch_arg, visualization_modules_launch_arg, rviz,
        launch_ros.actions.Node(package='uvms_visualization',
                                executable='visualization_node',
                                namespace=namespace,
                                remappings=[('/' + namespace + topic,
                                             vehicle_namespace + topic)
                                            for topic in visualization_topics],
                                parameters=[{
                                    'use_sim_time':
                                    use_sim_time,
