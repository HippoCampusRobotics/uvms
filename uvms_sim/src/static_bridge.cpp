...
public:
    Bridge() {
        ...
        if (!ros_node_->get_parameter(parameterName, simulate_kinematics_)) {
            RCLCPP_WARN(ros_node_->get_logger(), "%s",
                        ("Parameter " + parameterName + " not properly loaded, use default "
    ...
    void CreateClockBridge() {
        ...
        clock_pub_ = ros_node_->create_publisher<rosgraph_msgs::msg::Clock>("/clock", rclcpp::SystemDefaultsQoS());
    ...
    void CreateVelocityCommandBridge() {
        vel_cmd_sub_ = ros_node_->create_subscription<hippo_msgs::msg::VelocityControlTarget>("velocity_setpoint",
                                                                                       rclcpp::SystemDefaultsQoS(), std::bind(&Bridge::OnVelocityCommand, this, _1));
        vel_cmd_pub_ = gz_node_->Advertise<ignition::msgs::Twist>(std::string(ros_node_->get_namespace()) + "/vel_cmds");
    }

    void CreateJointAccelerationBridge() {
        joint_accel_pub_ = ros_node_->create_publisher<alpha_msgs::msg::JointData>("joint_accelerations_raw",
                                                                                   rclcpp::SystemDefaultsQoS());
        std::function<void(const ignition::msgs::Double_V &)> f = std::bind(&Bridge::OnJointAcceleration, this, _1);
        gz_node_->Subscribe(std::string(ros_node_->get_namespace()) + "/joint_accelerations", f);
    }
    ...
    void CreateJointStateBridge() {
        ...
        if (!ros_node_->get_parameter(parameterName, joint_state_update_frequency_)) {
            RCLCPP_WARN(ros_node_->get_logger(), "%s",
                        ("Parameter " + parameterName + " not properly loaded, use default "
                                                        "value " + std::to_string(default_value)).c_str());
        }
        std::function<void(const ignition::msgs::Model &)> f = std::bind(&Bridge::OnJointState, this, _1);
        gz_node_->Subscribe(std::string(ros_node_->get_namespace()) + "/joint_states", f);
        RCLCPP_INFO(ros_node_->get_logger(), "%s", (std::string(ros_node_->get_namespace()) + "/joint_states").c_str());
        joint_state_pub_ = ros_node_->create_publisher<sensor_msgs::msg::JointState>("joint_states_raw",
                                                                                     rclcpp::SystemDefaultsQoS());
    ...
    void CreateJointCmdBridge() {
        ...
        for (size_t i = 0; i < alpha_msgs::msg::JointData().data.size(); i++) {
            ...
            joint_cmd_pubs_[i] = gz_node_->Advertise<ignition::msgs::Double>(topic_name);
        }
        ...
        joint_cmd_sub_ = ros_node_->create_subscription<alpha_msgs::msg::JointData>(topic_sub_name,
                                                                                    rclcpp::SystemDefaultsQoS(),
                                                                                    std::bind(&Bridge::OnJointCommand,
                                                                                              this, _1));
    }

    void CreateBaseForceTorqueBridge() {
        base_ft_pub_ = ros_node_->create_publisher<geometry_msgs::msg::Wrench>("force_torque_gz",
                                                                               rclcpp::SystemDefaultsQoS());
        std::function<void(const ignition::msgs::Wrench &)> f = std::bind(&Bridge::OnBaseForceTorque, this, _1);
        gz_node_->Subscribe(std::string(ros_node_->get_namespace()) + "/force_torque_gz", f);
    }
    ...
    void OnVelocityCommand(const hippo_msgs::msg::VelocityControlTarget::SharedPtr ros_msg){
    ...
    void OnJointCommand(const alpha_msgs::msg::JointData::SharedPtr msg) {
        ...
        if (msg->data.size() != joint_cmd_pubs_.size()) {
            RCLCPP_ERROR(ros_node_->get_logger(), "Joint Command Message and num. of publishers aren't equal!");
            return;
        }
    ...
    void OnJointAcceleration(const ignition::msgs::Double_V &msg) {
        ...
        if (msg.data_size() > int(ros_msg.data.size())) {
            RCLCPP_ERROR(ros_node_->get_logger(),
                         "Ignition message size for joint accelerations greater than ros msg size");
        }
        ros_msg.header.stamp = ros_node_->now();
    ...
    void OnJointState(const ignition::msgs::Model &msg) {
        if ((ros_node_->now() - last_joint_state_update_).seconds() >= 1 / joint_state_update_frequency_) {
            ...
            for (auto i = 0; i < msg.joint_size(); ++i) {
                ...
                if (msg.joint(i).axis1().position() >= 0.0 ){
                    ...
                    ros_msg.position[i] = msg.joint(i).axis1().position() + (std::floor(std::abs(msg.joint(i).axis1().position()) /
                                                                                                (2 * M_PI)) + 1) * 2 * M_PI;
                }
...
private:
    ...
    rclcpp::Subscription<hippo_msgs::msg::VelocityControlTarget>::SharedPtr vel_cmd_sub_;
