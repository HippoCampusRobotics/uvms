...
class Estimation : public rclcpp::Node {
 public:
  Estimation() : Node("acceleration_estimation_node") {
    ...
    odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "odometry", qos,
        std::bind(&Estimation::odomCallback, this, std::placeholders::_1));
 ...
 private:
  ...
  std::vector<KFInterface*>
      kf_interfaces_;  //!< interfaces to KF, need only one but makes option
                       //!< available to compare multiple simultaneously
