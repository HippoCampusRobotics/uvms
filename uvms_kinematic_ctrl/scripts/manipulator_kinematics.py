...
def solve(manipulator: Manipulator, col_constr: [], joint_limits: [JointLimitConstraint], q0: np.ndarray, eps: float) -> (bool, np.ndarray):
    q = q0  # initialize
    ...
    while (f > eps) & (counter < 1000):
        ...
        for constraint in constraints:
            if constraint.isActive():
                ...
                dq += projector @ (jacobian_tmp.transpose() @ np.linalg.inv((jacobian_tmp @ jacobian_tmp.transpose())) @ constraint.getTaskError()).reshape(-1, 1)
                #projector = np.eye(4) - jacobian.transpose() @ np.linalg.inv((jacobian @ jacobian.transpose())) @ jacobian
                projector = projector @ (np.eye(4) - jacobian_tmp.transpose() @ np.linalg.inv((jacobian_tmp @ jacobian_tmp.transpose())) @ jacobian_tmp)
        q += dq
        manipulator.updateTrafos(q)
...
class Trafo:
    ...
    def fromRPY(self, vec : np.ndarray, r, p, y):
        self.p = vec
        self.R = np.array([[np.cos(y)*np.cos(p), np.cos(y)*np.sin(p)*np.sin(r) - np.cos(r)*np.sin(y), np.sin(y)*np.sin(r) + np.cos(y)*np.cos(r)*np.sin(p)],
            [np.cos(p)*np.sin(y), np.cos(y)*np.cos(r) + np.sin(y)*np.sin(p)*np.sin(r), np.cos(r)*np.sin(y)*np.sin(p) - np.cos(y)*np.sin(r)],
    ...
    def getEefPositionJacobian(self, J : np.ndarray):
        for i in range(0, 4):
            if i == 0:
                ...
                J[:, i] = np.cross(self.tfs[i-1].matrix()[0:3, 0:3] @ e3, self.tfs[-1].matrix()[0:3, 3].reshape(3, 1) - \
    ...
    def getLinkPositionJacobian(self, J : np.ndarray, idx):
        for i in range(0, 4):
            if i == 0:
                ...
                J[:, i] = np.cross(self.tfs[i-1].matrix()[0:3, 0:3] @ e3, self.tfs[idx].matrix()[0:3, 3].reshape(3, 1) - \
                          self.tfs[i-1].matrix()[0:3, 3].reshape(3, 1), axis=0).reshape(-1)
            else:
                J[:, i] = np.zeros(3, )
