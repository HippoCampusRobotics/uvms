...
namespace uvms_kin_ctrl {
    ...
    void TaskInterface::initialize(rclcpp::Node *node_ptr, std::mutex *mutex_ptr, const int &task_type,
    ...
    void TaskInterface::publishCmds() {
        ...
        if (getTaskPtr()->isActive()) {
            const Eigen::Matrix<double, n_states_uvms, 1> debug_task_vel = getTaskPtr()->getDesiredVelocities();
            std::copy(debug_task_vel.begin(), debug_task_vel.end(), msg.desired_velocity.begin());
            const Eigen::Matrix<double, n_states_uvms, 1> debug_projected_vel = getTaskPtr()->getProjectedVelocities();
            std::copy(debug_projected_vel.begin(), debug_projected_vel.end(), msg.projected_velocity.begin());
            ...
            std::copy(getTaskPtr()->getActivationMatrix().diagonal().begin(),
                      getTaskPtr()->getActivationMatrix().diagonal().end(), msg.activation_values.begin());
        } else {
            std::fill(msg.desired_velocity.begin(), msg.desired_velocity.end(), 0.0);
            std::fill(msg.projected_velocity.begin(), msg.projected_velocity.end(), 0.0);
            msg.active_tasks = 0;
            msg.activation_values.resize(getTaskPtr()->getFullTaskDimension());
    ...
    void JointLimitTaskInterface::initializeTask() {
        ...
        std::vector<std::string> description_suffix = {"joint 0", "joint 1", "joint 2", "joint 3"};
        ...
        for (int i = 0; i < int(params.size()); i++) {
            ...
            joint_limits_min(i) = node_ptr_->declare_parameter(name, joint_limits_min(i), descr);
        }
        ...
        for (int i = 0; i < int(params.size()); i++) {
            ...
            joint_limits_max(i) = node_ptr_->declare_parameter(name, joint_limits_max(i), descr);
        }
        ...
        task_.initialize(safety, delta, alpha, joint_limits_min, joint_limits_max, gains);

    }

    void JointLimitTaskInterface::addParameterCallback() {
        gains_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&JointLimitTaskInterface::onSetGains, this, std::placeholders::_1));

    }

    rcl_interfaces::msg::SetParametersResult
    JointLimitTaskInterface::onSetGains(const std::vector<rclcpp::Parameter> &parameters) {
        ...
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(*mutex_ptr_);
            for (int i = 0; i < int(params.size()); i++) {
                if (hippo_common::param_utils::AssignIfMatch(parameter,
                                                             "joint_limits.gain." + params[i],
                                                             gains_(i))) {
                    task_.setTaskGain(gains_(i), i);
                    RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                                std::to_string(gains_(i))).c_str());
                    break;
                }
            }
        }
    ...
    void JointLimitDesiredTaskInterface::addParameterCallback() {
        gains_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&JointLimitDesiredTaskInterface::onSetGains, this, std::placeholders::_1));

    }

    rcl_interfaces::msg::SetParametersResult
    JointLimitDesiredTaskInterface::onSetGains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        std::vector<std::string> params = {"q0", "q1", "q2", "q3"};
        ...
        for (const rclcpp::Parameter &parameter: parameters) {
            ...
            for (int i = 0; i < int(params.size()); i++) {
                if (hippo_common::param_utils::AssignIfMatch(parameter,
                                                             "joint_limits_desired.gain." + params[i],
                                                             gains_(i))) {
                    task_.setTaskGain(gains_(i), i);
                    RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                                std::to_string(gains_(i))).c_str());
    ...
    void JointCenteringTaskInterface::addParameterCallback() {
        gains_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&JointCenteringTaskInterface::onSetGains, this, std::placeholders::_1));

    }

    rcl_interfaces::msg::SetParametersResult
    JointCenteringTaskInterface::onSetGains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        std::vector<std::string> params = {"q0", "q1", "q2", "q3"};
    ...
    void VelocityTaskInterface::initializeTask() {
        task_.initialize();
    }
    ...
    void SelfCollisionEllipseTaskInterface::initializeTask() {
        ...
        std::lock_guard<std::mutex> lock(*mutex_ptr_);
        task_.initialize(idxs_int, safety, delta, alpha, a[0], a[1], c[0], c[1], gain);
    }

    void SelfCollisionEllipseTaskInterface::addParameterCallback() {
        gain_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&SelfCollisionEllipseTaskInterface::onSetGains, this, std::placeholders::_1));

    }

    rcl_interfaces::msg::SetParametersResult
    SelfCollisionEllipseTaskInterface::onSetGains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(*mutex_ptr_);
            if (hippo_common::param_utils::AssignIfMatch(parameter,
                                                         "ellipse_collision.gain", gain_)) {
                task_.setTaskGain(gain_);
                RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                            std::to_string(gain_)).c_str());
                break;
            }
    ...
    void AUVConfinedSpaceTaskInterface::initializeTask() {
        ...
        if ((points[0].size() != points[1].size()) || (points[1].size() != points[2].size()) ||
            ...
            RCLCPP_ERROR(node_ptr_->get_logger(), "Input point sizes for AUV plane constraints not consistent");
            return;
        }
        ...
        for (int i = 0; i < int(points[0].size()); i++) {
            Eigen::Vector3d point = Eigen::Vector3d(points[0][i], points[1][i], points[2][i]);
            Eigen::Vector3d normal = Eigen::Vector3d(normals[0][i], normals[1][i], normals[2][i]);
    ...
    void RestrictingPlaneTaskInterface::initializeTask() {
        ...
        for (int i = 0; i < int(params.size()); i++) {
            ...
            descr_text = "Position for vertical plane, direction" + description_suffix[i];
            descr = hippo_common::param_utils::Description(descr_text);
        ...
        for (int i = 0; i < int(params.size()); i++) {
            ...
            descr_text = "Direction for vertical plane, direction" + description_suffix[i];
            descr = hippo_common::param_utils::Description(descr_text);
    ...
    void RestrictingPlaneTaskInterface::addParameterCallback() {
        gain_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&RestrictingPlaneTaskInterface::onSetGains, this, std::placeholders::_1));

    }

    rcl_interfaces::msg::SetParametersResult
    RestrictingPlaneTaskInterface::onSetGains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(*mutex_ptr_);
            if (hippo_common::param_utils::AssignIfMatch(parameter,
                                                         "restricting_plane.gain", gain_)) {
                task_.setTaskGain(gain_);
                RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                            std::to_string(gain_)).c_str());
                break;
    ...
    void AUVConfinedSpaceTaskInterface::addParameterCallback() {
        ...
        gain_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&AUVConfinedSpaceTaskInterface::onSetGains, this, std::placeholders::_1));
    }


    rcl_interfaces::msg::SetParametersResult
    AUVConfinedSpaceTaskInterface::onSetGains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(*mutex_ptr_);
            if (hippo_common::param_utils::AssignIfMatch(parameter,
                                                         "auv_confined_space.gain", gain_)) {
                task_.setTaskGain(gain_);
                RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                            std::to_string(gain_)).c_str());
                break;
    ...
    void AUVAttitudeLimitTaskInterface::initializeTask() {
        ...
        for (int i = 0; i < int(idxs.size()); i++) {
            ...
            attitude_limits_min[i] = node_ptr_->declare_parameter(name, attitude_limits_min[i], descr);
        ...
        for (int i = 0; i < int(idxs.size()); i++) {
            ...
            attitude_limits_max[i] = node_ptr_->declare_parameter(name, attitude_limits_max[i], descr);
        ...
        std::lock_guard<std::mutex> lock(*mutex_ptr_);
        task_.initialize(idxs, delta, alpha, attitude_limits_min, attitude_limits_max, gains);

    }

    void AUVAttitudeLimitTaskInterface::addParameterCallback() {
        gains_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&AUVAttitudeLimitTaskInterface::onSetGains, this, std::placeholders::_1));

    }

    rcl_interfaces::msg::SetParametersResult
    AUVAttitudeLimitTaskInterface::onSetGains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        std::vector<std::string> params = {"roll", "pitch", "yaw"};
    ...
    void AUVAttitudeInclinationLimitTaskInterface::addParameterCallback() {
        gain_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&AUVAttitudeInclinationLimitTaskInterface::onSetGain, this, std::placeholders::_1));

    }

    rcl_interfaces::msg::SetParametersResult
    AUVAttitudeInclinationLimitTaskInterface::onSetGain(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        ...
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(*mutex_ptr_);
            if (hippo_common::param_utils::AssignIfMatch(parameter,
                                                         "attitude_inclination_limits.gain",
                                                         gain_)) {
                task_.setTaskGain(gain_);
                RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                            std::to_string(gain_)).c_str());
    ...
    void EndeffectorTrackingTaskInterface::initializeTask() {
        ...
        std::vector<std::string> params = {"pos.x", "pos.y", "pos.z", "att.x", "att.y", "att.z"};
        std::vector<std::string> description_suffix = {"pos x", "pos y", "pos z", "att x", "att y", "att z"};
        std::vector<double> default_params;
        ...
        for (int i = 0; i < int(params.size()); i++) {
            ...
            descr_text = "Proportional gain for endeffector kinematic control " + description_suffix[i];
            descr = hippo_common::param_utils::Description(descr_text);
            {
                std::lock_guard<std::mutex> lock(*mutex_ptr_);
    ...
    void EndeffectorTrackingTaskInterface::addParameterCallback() {
        ...
        p_gains_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&EndeffectorTrackingTaskInterface::onSetPgains, this, std::placeholders::_1));


    }

    rcl_interfaces::msg::SetParametersResult
    EndeffectorTrackingTaskInterface::onSetPgains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        std::vector<std::string> params = {"pos.x", "pos.y", "pos.z", "att.x", "att.y", "att.z"};
        ...
        for (const rclcpp::Parameter &parameter: parameters) {
            ...
            for (int i = 0; i < int(params.size()); i++) {
                if (hippo_common::param_utils::AssignIfMatch(parameter,
                                                             "eef_tracking.task_error_gain." + params[i],
                                                             p_gains_(i))) {
                    task_.setEefGain(p_gains_(i), i);
                    RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                                std::to_string(p_gains_(i))).c_str());
                    break;
                }
            }
        }
    ...
    void ManipulabilityTaskInterface::addParameterCallback() {
        gain_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&ManipulabilityTaskInterface::onSetGains, this, std::placeholders::_1));

    }

    rcl_interfaces::msg::SetParametersResult
    ManipulabilityTaskInterface::onSetGains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        for (const rclcpp::Parameter &parameter: parameters) {
            if (hippo_common::param_utils::AssignIfMatch(parameter, "manipulability.gain", gain_)) {
                std::lock_guard<std::mutex> lock(*mutex_ptr_);
                task_.setTaskGain(gain_);
                RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                            std::to_string(gain_)).c_str());
    ...
    void ManipulabilityLimitTaskInterface::addParameterCallback() {
        gain_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&ManipulabilityLimitTaskInterface::onSetGains, this, std::placeholders::_1));

    }

    rcl_interfaces::msg::SetParametersResult
    ManipulabilityLimitTaskInterface::onSetGains(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(*mutex_ptr_);
            if (hippo_common::param_utils::AssignIfMatch(parameter, "manipulability_limit.gain", gain_)) {
                task_.setTaskGain(gain_);
                RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                            std::to_string(gain_)).c_str());
                break;
    ...
    void UVMSKinematicControlInterface::initializeController() {
        ...
        for (size_t i = 0; i < param_utils::n_links; i++) {
            RCLCPP_INFO(node_ptr_->get_logger(), "%s", static_cast<std::string>(param_utils::link_names[i]).c_str());
            ros_param_utils::loadLinkTFParams(node_ptr_, dummy_active, dummy_inertial, tf_params[i],
        ...
        publish_task_velocities_ = node_ptr_->declare_parameter("publish_task_velocities", publish_task_velocities_,
                                                                descr);

        controller_.initialize(tf_params, algorithm_type);

        std::vector<std::string> params = {"linear.x", "linear.y", "linear.z", "angular.x", "angular.y", "angular.z",
                                           "joints.q0", "joints.q1", "joints.q2", "joints.q3"};
        std::vector<std::string> description_suffix = {"linear x", "linear y", "linear z", "angular x", "angular y",
        ...
        for (int i = 0; i < int(params.size()); i++) {
            ...
            velocity_limits(i) = node_ptr_->declare_parameter(name, velocity_limits(i), descr);
        }
        ...
        accel_smooth_fac_min = node_ptr_->declare_parameter(name, accel_smooth_fac_min, descr);
        ...
        accel_smooth_fac_max = node_ptr_->declare_parameter(name, accel_smooth_fac_max, descr);

        double delta_accel_min;
        name = "delta_accel_min";
        descr_text = "Minimum acceleration difference between time steps to apply smoothing factor";
        descr = hippo_common::param_utils::Description(descr_text);
        delta_accel_min = 0.3;
        ...
        descr_text = "Maximum acceleration difference between time steps to apply smoothing factor";
        descr = hippo_common::param_utils::Description(descr_text);
        delta_accel_max = 1.0;
        delta_accel_max = node_ptr_->declare_parameter(name, delta_accel_max, descr);

        controller_.setAccelerationSmoothingFactors(accel_smooth_fac_min, accel_smooth_fac_max, delta_accel_min,
                                                    delta_accel_max);
        double eta;
        name = "eta";
        descr_text = "Factor for shifting focus onto minimizing control directions in transition";
        descr = hippo_common::param_utils::Description(descr_text);
        eta = 1.0;
        ...
        descr_text = "Decides if weighting is applied to whole Nullspace projection or only task-wise";
        descr = hippo_common::param_utils::Description(descr_text);
        nullspace_weighting = false;
        nullspace_weighting = node_ptr_->declare_parameter(name, nullspace_weighting, descr);
        ...
        std::vector<int64_t> task_keys = {TaskKey::joint_limits, TaskKey::endeffector_tracking};
        ...
        params = {"pos.x", "pos.y", "pos.z", "att.x", "att.y", "att.z", "joint.q0", "joint.q1", "joint.q2", "joint.q3"};
        description_suffix = {"pos x", "pos y", "pos z", "att x", "att y", "att z", "joint q0", "joint q1", "joint q2",
                              "joint q3"};
        default_params.resize(params.size());
        std::fill(default_params.begin(), default_params.end(), 1.0);
        for (int i = 0; i < int(params.size()); i++) {
            name = "weighting_matrix." + params[i];
            descr_text = "Weighting parameter for motion along " + description_suffix[i];
            descr = hippo_common::param_utils::Description(descr_text);
            {
                std::lock_guard<std::mutex> lock(mutex_);
                weighting_params_(i) = default_params[i];
                weighting_params_(i) = node_ptr_->declare_parameter(name, weighting_params_(i), descr);
            }
        }
        ...
        for (int task_idx = 0; task_idx < int(task_keys.size()); task_idx++) {
            switch (task_keys[task_idx]) {
                ...
                case TaskKey::endeffector_tracking:
                    ...
                    dynamic_cast<EndeffectorTrackingTaskInterface *>(task_interfaces_.back())->addReferences(
                            controller_.getPositionTargetReference(), controller_.getAttitudeTargetReference(),
                            controller_.getVelocitiesTargetReference(), controller_.getTrackingMaskReference());
                    break;
                ...
                case TaskKey::auv_attitude_inclination_limits:
                    task_interfaces_.push_back(new AUVAttitudeInclinationLimitTaskInterface());
                    break;
                ...
                default:
                    RCLCPP_ERROR(node_ptr_->get_logger(), "%s",
                                 ("Chosen task key is " + std::to_string(task_keys[task_idx]) +
                                  " which is not a valid task key!").c_str());
                    break;
        ...
        for (int task_idx = 0; task_idx < int(task_keys.size()); task_idx++) {
            task_interfaces_[task_idx]->getTaskPtr()->setIsContinuous(algorithm_type == AlgorithmType::continuous);
            ...
            task_interfaces_[task_idx]->initialize(node_ptr_, &mutex_, int(task_keys[task_idx]),
                                                   publish_task_velocities_);
            task_interfaces_[task_idx]->addReferenceModel(controller_.getKinematicsPtr());
            controller_.addTask(task_interfaces_[task_idx]->getTaskPtr(), TaskType(task_types[task_idx]));
        }

        // set here, as this also sets the weighting parameters for the task velocities for the last velocity task, which must be initialized first!
    ...
    void UVMSKinematicControlInterface::addParameterCallback() {
        svd_param_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&UVMSKinematicControlInterface::onSetSVDParam, this, std::placeholders::_1));
        weighting_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&UVMSKinematicControlInterface::onSetWeightingParams, this, std::placeholders::_1));
        eta_param_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&UVMSKinematicControlInterface::onSetEta, this, std::placeholders::_1));
        accel_smoothing_cb_handle_ = node_ptr_->add_on_set_parameters_callback(
                std::bind(&UVMSKinematicControlInterface::onSetAccelSmoothingFactors, this, std::placeholders::_1));
    }

    rcl_interfaces::msg::SetParametersResult
    UVMSKinematicControlInterface::onSetSVDParam(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        ...
        for (const rclcpp::Parameter &parameter: parameters) {
            ...
            if (hippo_common::param_utils::AssignIfMatch(parameter,
                ...
                RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                            std::to_string(sigma_min)).c_str());
            } else if (hippo_common::param_utils::AssignIfMatch(parameter,
                                                                "svd_damping",
                                                                k_svd_damping)) {
                for (TaskInterface *task_interface: task_interfaces_) {
                ...
                RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                            std::to_string(k_svd_damping)).c_str());
            }
        }
        return result;
    }
    ...
    rcl_interfaces::msg::SetParametersResult
    UVMSKinematicControlInterface::onSetWeightingParams(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        std::vector<std::string> params = {"pos.x", "pos.y", "pos.z", "att.x", "att.y", "att.z", "joint.q0",
                                           "joint.q1", "joint.q2", "joint.q3"};
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(mutex_);
            for (int i = 0; i < int(params.size()); i++) {
                if (hippo_common::param_utils::AssignIfMatch(parameter,
                                                             "weighting_matrix." + params[i],
                                                             weighting_params_(i))) {
                    controller_.setWeightingParam(weighting_params_(i), i);
                    RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                                std::to_string(weighting_params_(i))).c_str());
                    break;
                }
            }
        }
    ...
    rcl_interfaces::msg::SetParametersResult
    UVMSKinematicControlInterface::onSetEta(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        double eta;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(mutex_);
            if (hippo_common::param_utils::AssignIfMatch(parameter, "eta",
                                                         eta)) {
                controller_.setEta(eta);
                RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                            std::to_string(eta)).c_str());
                break;
            }
        }
        return result;
    ...
    rcl_interfaces::msg::SetParametersResult
    UVMSKinematicControlInterface::onSetAccelSmoothingFactors(const std::vector<rclcpp::Parameter> &parameters) {
        rcl_interfaces::msg::SetParametersResult result;
        result.successful = true;
        double accel_smoothing_factor_min, accel_smoothing_factor_max;
        for (const rclcpp::Parameter &parameter: parameters) {
            std::lock_guard<std::mutex> lock(mutex_);
            if (hippo_common::param_utils::AssignIfMatch(parameter, "accel_smooth_fac_min",
                                                         accel_smoothing_factor_min)) {
                controller_.setAccelSmoothingFactorMin(accel_smoothing_factor_min);
                RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                            std::to_string(accel_smoothing_factor_min)).c_str());
                break;
            }
            if (hippo_common::param_utils::AssignIfMatch(parameter, "accel_smooth_fac_max",
                                                         accel_smoothing_factor_max)) {
                controller_.setAccelSmoothingFactorMax(accel_smoothing_factor_max);
                RCLCPP_INFO(node_ptr_->get_logger(), "%s", ("Changed parameter " + parameter.get_name() + " to " +
                                                            std::to_string(accel_smoothing_factor_max)).c_str());
                break;
            }
        }
        return result;
    ...
    void UVMSKinematicControlInterface::getEndeffectorPose(geometry_msgs::msg::Pose &out_msg) {
        controller_.getEefPosition(out_msg.position.x, out_msg.position.y, out_msg.position.z);
        controller_.getEefAttitude(out_msg.orientation.w, out_msg.orientation.x, out_msg.orientation.y,
                                   out_msg.orientation.z);
    }

    void UVMSKinematicControlInterface::getEndeffectorPose(const nav_msgs::msg::Odometry &auv_msg,
        ...
        for (size_t i = 0; i < n_active_joints; i++) {
            auto entry = std::find(manipulator_msg.name.begin(), manipulator_msg.name.end(),
            ...
            if (entry != std::end(manipulator_msg.name)) {
                ...
                RCLCPP_ERROR(node_ptr_->get_logger(), "Requested joint not found in published states");
                return;
            }
        }
    ...
    void UVMSKinematicControlInterface::setSetpointTargetForward(
        ...
        hippo_common::convert::RosToEigen(_msg->angular_velocity, ang_vel_des);
        std::lock_guard<std::mutex> lock(mutex_);
        controller_.setControlTargetForward(pos_des, att_des, vel_des, ang_vel_des, dt);
    }

    void UVMSKinematicControlInterface::getControllerOutput(const nav_msgs::msg::Odometry &auv_msg,
                                                            const sensor_msgs::msg::JointState &manipulator_msg,
        ...
        param_utils::UVMSStateVector cmd_vel;
        {
            ...
            //std::cout << std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now() - start).count()*1e-9 << std::endl;
        }
        const Eigen::Ref<const Eigen::Vector3d> ref_auv_vel = cmd_vel.segment<3>(0);
        const Eigen::Ref<const Eigen::Vector3d> ref_auv_ang_vel = cmd_vel.segment<3>(3);
        hippo_common::convert::EigenToRos(ref_auv_vel, out_auv_cmds.twist.linear);
        hippo_common::convert::EigenToRos(ref_auv_ang_vel, out_auv_cmds.twist.angular);
        std::copy(cmd_vel.segment<n_active_joints>(6).begin(), cmd_vel.segment<n_active_joints>(6).end(),
    ...
    void UVMSKinematicControlInterface::getControllerOutputWithDerivative(const double &dt,
        ...
        const Eigen::Ref<const Eigen::Vector3d> ref_auv_ang_acc = cmd_acc.segment<3>(3);
