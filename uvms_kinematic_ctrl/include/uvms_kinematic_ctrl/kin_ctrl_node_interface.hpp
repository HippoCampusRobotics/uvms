...
class TaskInterface {
 public:
  ...
  void initialize(rclcpp::Node *node_ptr, std::mutex *mutex_ptr,
                  const int &task_type, const bool &publish_task_velocities);
...
class JointLimitTaskInterface : public TaskInterface {
 public:
  ...
  Task *getTaskPtr() override { return &task_; }
 ...
 private:
  ...
  rcl_interfaces::msg::SetParametersResult onSetGains(
...
class SelfCollisionEllipseTaskInterface : public TaskInterface {
 ...
 private:
  ...
  SelfCollisionEllipseTask task_;  //!< two tasks as collision is tested for two
...
class RestrictingPlaneTaskInterface : public TaskInterface {
 ...
 private:
  ...
  rcl_interfaces::msg::SetParametersResult onSetGains(
      const std::vector<rclcpp::Parameter> &parameters);
  RestrictingPlaneTask task_;  //!< two tasks as collision is tested for two
                               //!< links of the manipulator
  double gain_;
  rclcpp::node_interfaces::OnSetParametersCallbackHandle::SharedPtr
      gain_cb_handle_;
...
class AUVConfinedSpaceTaskInterface : public TaskInterface {
 ...
 private:
  ...
  AUVConfinedSpaceTask task_;  //!< two tasks as collision is tested for two
                               //!< links of the manipulator
  double gain_;
  rclcpp::node_interfaces::OnSetParametersCallbackHandle::SharedPtr
      gain_cb_handle_;
...
class AUVAttitudeInclinationLimitTaskInterface : public TaskInterface {
 ...
 private:
  ...
  rcl_interfaces::msg::SetParametersResult onSetGain(
      const std::vector<rclcpp::Parameter> &parameters);
...
class UVMSKinematicControlInterface {
 public:
  ...
  //! Request end-effector pose, assumes that the pose was manually updated
  //! before based on given state information \param out end-effector position
  //! and orientation in world-COS
  ...
  void getControllerOutputWithDerivative(
  ...
  rcl_interfaces::msg::SetParametersResult onSetWeightingParams(
      const std::vector<rclcpp::Parameter> &parameters);

  rcl_interfaces::msg::SetParametersResult onSetAccelSmoothingFactors(
      const std::vector<rclcpp::Parameter> &parameters);
 ...
 private:
  ...
  rcl_interfaces::msg::SetParametersResult onSetSVDParam(
      const std::vector<rclcpp::Parameter> &parameters);
  rcl_interfaces::msg::SetParametersResult onSetEta(
      const std::vector<rclcpp::Parameter> &parameters);

  rclcpp::Node *node_ptr_;
  bool publish_task_velocities_;  //!< decides if for each task a publisher is
                                  //!< created that publishes the
                                  //!< (unsaturated!!) cmd vel
  UVMSKinematicCtrl controller_;
  std::vector<TaskInterface *> task_interfaces_;
  std::mutex mutex_;
  rclcpp::node_interfaces::OnSetParametersCallbackHandle::SharedPtr
      svd_param_cb_handle_;
  rclcpp::node_interfaces::OnSetParametersCallbackHandle::SharedPtr
      eta_param_cb_handle_;
  UVMSStateVector weighting_params_;
  rclcpp::node_interfaces::OnSetParametersCallbackHandle::SharedPtr
      weighting_cb_handle_;
  rclcpp::node_interfaces::OnSetParametersCallbackHandle::SharedPtr
      accel_smoothing_cb_handle_;
};
