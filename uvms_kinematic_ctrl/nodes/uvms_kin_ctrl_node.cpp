...
namespace uvms_kin_ctrl {
    UVMSKinematicControlNode::UVMSKinematicControlNode(): Node("uvms_kin_ctrl_node")
    {
    ...
    void UVMSKinematicControlNode::initPublishers() {
        ...
        manipulator_cmd_pub_ = create_publisher<alpha_msgs::msg::JointData>(
                topic, qos);
        topic = "velocity_setpoint";
        auv_vel_cmd_pub_ = create_publisher<hippo_msgs::msg::VelocityControlTarget>(topic, qos);
    ...
    void UVMSKinematicControlNode::declareParams() {
        ...
        ros_param_utils::getParam(this, publish_on_joint_state_, "publish_on_joint_state", false);
    }
    ...
    void UVMSKinematicControlNode::initSubscriptions() {
        ...
        state_sub_manipulator_ = create_subscription<sensor_msgs::msg::JointState>(
                topic, qos,
                std::bind(&UVMSKinematicControlNode::onJointState, this, _1));
        ...
        if (!use_prediction_) {
            topic = "traj_setpoint";
            eef_traj_sub_ = create_subscription<hippo_msgs::msg::ControlTarget>(topic, qos,
                                                                                std::bind(
                                                                                        &UVMSKinematicControlNode::onSetpointTarget,
                                                                                        this, _1));
        } else {
            topic = "traj_setpoint_prediction";
            eef_traj_sub_prediction_ = create_subscription<uvms_msgs::msg::ControlTargetPrediction>(topic, qos,
                                                                                                    std::bind(
                                                                                                            &UVMSKinematicControlNode::onSetpointTargetPrediction,
                                                                                                            this, _1));
        }
    }

    alpha_msgs::msg::JointData UVMSKinematicControlNode::zeroManipulatorMsg(const rclcpp::Time &_stamp) {
    ...
    void UVMSKinematicControlNode::onSetpointTimeout() {
        ...
        RCLCPP_WARN(get_logger(), "Endeffector trajectory setpoint timed out. Sending zero commands.");
    ...
    void UVMSKinematicControlNode::onSetpointTarget(const hippo_msgs::msg::ControlTarget::SharedPtr _msg) {
        if (_msg->header.frame_id != "map") {
            RCLCPP_WARN_THROTTLE(
                    get_logger(), *get_clock(), 1000,"%s",
                    ("Endeffector trajectory frame is " + std::string(_msg->header.frame_id) + " but only [" + std::string(hippo_common::tf2_utils::frame_id::kInertialName) +"] is handled. Ignoring..." +
        ...
        if (controller_status_ != ControllerStatus::eef_control){
            RCLCPP_INFO(this->get_logger(),
                        "Activating end effector controller.");
            controller_status_ = ControllerStatus::eef_control;
            RCLCPP_INFO(this->get_logger(), "Deactivating configuration space controller...");
        ...
        if (setpoint_timed_out_) {
            RCLCPP_INFO(get_logger(),
                        "Received endeffector trajectory setpoint. Setpoint not timed out anymore.");
            setpoint_timed_out_ = false;
        }
        controller_interface_->setSetpointTarget(_msg);
    }

    void UVMSKinematicControlNode::onSetpointTargetPrediction(const uvms_msgs::msg::ControlTargetPrediction::SharedPtr _msg) {
        if (_msg->header.frame_id != "map") {
            RCLCPP_WARN_THROTTLE(
                    get_logger(), *get_clock(), 1000,"%s",
                    ("Endeffector trajectory frame is " + std::string(_msg->header.frame_id) + " but only [" + std::string(hippo_common::tf2_utils::frame_id::kInertialName) +"] is handled. Ignoring..." +
        ...
        hippo_msgs::msg::ControlTarget::SharedPtr target_ptr = std::make_shared<hippo_msgs::msg::ControlTarget>(target);
    ...
    void UVMSKinematicControlNode::onOdometry(const nav_msgs::msg::Odometry::SharedPtr _msg) {
        last_odometry_ = *_msg;
    ...
    void UVMSKinematicControlNode::onJointState(const sensor_msgs::msg::JointState::SharedPtr _msg) {
    ...
    void UVMSKinematicControlNode::publishControlCmds() {
        ...
        switch (controller_status_) {
            case ControllerStatus::undeclared:
                ...
                manipulator_cmd_pub_->publish(joint_vel_msg);
                controller_interface_->getEndeffectorPose(last_odometry_, last_joint_state_, eef_pose.pose);
                break;
            case ControllerStatus::joint_space_control:
                if (!startup_controller_) {
                    RCLCPP_ERROR(this->get_logger(),
                                 "Requested startup controller, although it is already deleted, this should never happen!");
                }
                startup_controller_->publishControlCommands(last_odometry_, last_joint_state_);
                controller_interface_->getEndeffectorPose(last_odometry_, last_joint_state_, eef_pose.pose);
                break;
            case ControllerStatus::eef_control:
                if (setpoint_timed_out_ | !got_first_time_stamp_) {
                    ...
                    auv_vel_msg.header.frame_id = hippo_common::tf2_utils::frame_id::BaseLink(this);
                    auv_vel_msg.use_acceleration = true;
                    joint_vel_msg.header.stamp = auv_vel_msg.header.stamp;
                    controller_interface_->getControllerOutputWithDerivative(dt,
                                                                             last_odometry_, last_joint_state_,
                                                                             auv_vel_msg.velocity,
                                                                             auv_vel_msg.acceleration,
                                                                             joint_vel_msg);
                ...
                break;
            default:
                RCLCPP_ERROR(this->get_logger(), "Unknown controller status, sending zero manipulator velocities!");
                joint_vel_msg = zeroManipulatorMsg(now());
                manipulator_cmd_pub_->publish(joint_vel_msg);
                controller_interface_->getEndeffectorPose(last_odometry_, last_joint_state_, eef_pose.pose);
                break;
