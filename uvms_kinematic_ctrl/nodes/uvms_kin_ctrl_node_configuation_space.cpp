...
namespace uvms_kin_ctrl{
    ...
    void UVMSKinematicConfigurationControl::publishControlCommands(const nav_msgs::msg::Odometry &auv_msg,
        ...
        out_auv_msg.header.frame_id = hippo_common::tf2_utils::frame_id::BaseLink(node_ptr_);
        auv_position_controller_interface_->update(auv_msg.pose.pose.position, auv_msg.pose.pose.orientation, out_auv_msg.velocity.linear);
        out_auv_msg.acceleration.linear.x = 0.0;
        out_auv_msg.acceleration.linear.y = 0.0;
        out_auv_msg.acceleration.linear.z = 0.0;
        auv_attitude_controller_interface_->update(auv_msg.pose.pose.orientation, out_auv_msg.velocity.angular);
        ...
        sensor_msgs::msg::JointState::SharedPtr manipulator_msg_ptr = std::make_shared<sensor_msgs::msg::JointState>(manipulator_msg);
        manipulator_controller_interface_->update(manipulator_msg_ptr, out_manipulator_msg);
    ...
    void UVMSKinematicConfigurationControl::initController() {
        ...
        auv_attitude_controller_interface_ = new bluerov_ctrl::AttSkewSymmetricPModuleInterface();
        manipulator_controller_interface_ = new alpha_ctrl::JointKinematicControlInterface();
    ...
    void UVMSKinematicConfigurationControl::initSubscriptions() {
        ...
        setpoint_sub_ = node_ptr_->create_subscription<uvms_msgs::msg::UVMSControlTarget>(topic, qos,
                                                                            std::bind(&UVMSKinematicConfigurationControl::onSetpointTarget, this, _1));

    }

    alpha_msgs::msg::JointData UVMSKinematicConfigurationControl::zeroManipulatorMsg(const rclcpp::Time &_stamp) {
    ...
    void UVMSKinematicConfigurationControl::onSetpointTimeout() {
        ...
        RCLCPP_WARN(node_ptr_->get_logger(), "Configuration space setpoint timed out. Sending zero commands.");
    ...
    void UVMSKinematicConfigurationControl::onSetpointTarget(const uvms_msgs::msg::UVMSControlTarget::SharedPtr _msg) {
        if (_msg->header.frame_id != hippo_common::tf2_utils::frame_id::kInertialName) {
            RCLCPP_WARN_THROTTLE(
                    node_ptr_->get_logger(), *node_ptr_->get_clock(), 1000, "%s",
                    ("Configuration trajectory frame is [%s] but only [" + std::string(hippo_common::tf2_utils::frame_id::kInertialName) +"] is handled. Ignoring..." +
        ...
        if (setpoint_timed_out_) {
            RCLCPP_INFO(node_ptr_->get_logger(),
                        "Received configuration space setpoint. Setpoint not timed out anymore.");
            setpoint_timed_out_ = false;
        }

        hippo_msgs::msg::ControlTarget auv_setpoint = _msg->auv;
        hippo_msgs::msg::ControlTarget::SharedPtr auv_setpoint_ptr = std::make_shared<hippo_msgs::msg::ControlTarget>(auv_setpoint);
        ...
        manipulator_controller_interface_->setPositionTarget(_msg->manipulator.position);
        manipulator_controller_interface_->setVelocityTarget(_msg->manipulator.velocity);
    }
