...
bool loadLinkTFParams(bool &active, bool &inertial, param_utils::TFParam &tf, YAML::Node param){
...
int main(int argc, char** argv){
    ...
    loadLinkTFParams(dummy_active, dummy_inertial, tf_params[0],
    ...
    for (size_t i = 1; i < param_utils::n_links; i++){
        loadLinkTFParams(dummy_active, dummy_inertial, tf_params[i],
    ...
    std::cout << "Endeffector pos: " << "x: " << p_eef(0) << " y: " << p_eef(1) << " z: " << p_eef(2) << std::endl;

    Eigen::Quaterniond att_eef_des = att_eef * Eigen::AngleAxisd(0.3, Eigen::Vector3d::UnitX());
    ...
    Eigen::Matrix<double, param_utils::n_states_uvms, 6> inverse = J_tmp.transpose() * (J_tmp * J_tmp.transpose()).inverse();
    Eigen::Matrix<double, param_utils::n_states_uvms, 6> inverse_weighted = W.inverse() * J_tmp.transpose() * (J_tmp * W.inverse() * J_tmp.transpose()).inverse();
    Eigen::Vector<double, param_utils::n_states_uvms> vel_des = inverse * error;
    Eigen::Vector<double, param_utils::n_states_uvms> vel_des_weighted = inverse_weighted * error;
    Eigen::Vector<double, 6> check_eef = J_tmp * vel_des;
    Eigen::Vector<double, 6> check_eef_weighted = J_tmp * vel_des_weighted;

    Eigen::Quaterniond test_quaternion = Eigen::AngleAxisd(0.1, Eigen::Vector3d::UnitZ()) * Eigen::AngleAxisd(0.2, Eigen::Vector3d::UnitY()) *
            Eigen::AngleAxisd(0.3, Eigen::Vector3d::UnitX());
    Eigen::Vector3d test_rpy = test_quaternion.toRotationMatrix().eulerAngles(2, 1, 0).reverse();
    std::cout << "test_rpy" << test_rpy << std::endl;

    Eigen::Vector3d test_error;
    uvms_kinematics::quaternionError(Eigen::Quaterniond(Eigen::AngleAxisd(0.3, Eigen::Vector3d::UnitZ())), Eigen::Quaterniond::Identity(), test_error);
    ...
    for (int i = 0; i < int(param_utils::n_states_uvms); i++) {
        ...
        std::cout << "delta position" << (dp_eef - dp_eef2).norm() / eps << std::endl;
        ...
        std::cout << "delta rotation" << (drot_eef - drot_eef2).norm() / eps<< std::endl;
    }
    return 0;
}